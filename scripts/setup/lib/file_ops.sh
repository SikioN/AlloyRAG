...
# All environment keys the wizard may inject into the alloyrag service via
...
# Remove wizard-managed keys from the alloyrag service's environment block,
# leaving any user-added keys intact.
_strip_alloyrag_wizard_environment_keys() {
  local compose_file="$1"
  local tmp_file="${compose_file}.strip-wizard-keys"
  _FILE_OPS_CLEANUP_TMP+=("$tmp_file")
  local line key wk list_entry
  local in_alloyrag="no"
  local in_environment="no"

  : > "$tmp_file"

  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$line" == "  alloyrag:" ]]; then
      in_alloyrag="yes"
      in_environment="no"
    elif [[ "$in_alloyrag" == "yes" && "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ]]; then
      in_alloyrag="no"
      in_environment="no"
    fi

    if [[ "$in_alloyrag" == "yes" && "$line" == "    environment:" ]]; then
      in_environment="yes"
      printf '%s\n' "$line" >> "$tmp_file"
      continue
    fi

    if [[ "$in_alloyrag" == "yes" && "$in_environment" == "yes" ]]; then
      if [[ "$line" =~ ^[[:space:]]{6}([A-Z0-9_]+): ]]; then
        key="${BASH_REMATCH[1]}"
        for wk in "${_WIZARD_COMPOSE_ALLOYRAG_KEYS[@]}"; do
          if [[ "$key" == "$wk" ]]; then
...
_strip_wizard_managed_services_and_top_level_volumes() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # alloyrag, user-added services, and unchanged managed service groups.
    if [[ "$in_services" == "yes" && -n "$current_service" ]] && \
      [[ "$current_service" != "alloyrag" ]] && \
...
generate_docker_compose() {
  ...
  local template_file
  local alloyrag_mounts=()
  local alloyrag_env_entries=()
  local key
  local root_service

  # Prefer the existing generated compose as the starting point to preserve
  # any user customisations to the alloyrag service.  Fall back to the base
  ...
  prepare_alloyrag_service_for_generated_compose "$tmp_file"
  normalize_alloyrag_restart_policy "$tmp_file"
  # Remove stale wizard-managed keys from alloyrag's environment so that
  # keys no longer in COMPOSE_ENV_OVERRIDES are not left behind.
  _strip_alloyrag_wizard_environment_keys "$tmp_file"

  # Remove stale wizard-managed bind mounts from alloyrag's volumes so that
  # mounts no longer needed (e.g. after SSL removal) are not left behind.
  _strip_alloyrag_wizard_bind_mounts "$tmp_file"

  if [[ -n "${ALLOYRAG_COMPOSE_SERVER_PORT_MAPPING:-}" ]]; then
    _strip_alloyrag_wizard_ports "$tmp_file"
    inject_alloyrag_port_mapping "$tmp_file" "$ALLOYRAG_COMPOSE_SERVER_PORT_MAPPING"
  fi
  ...
  if ! _alloyrag_volumes_have_container_target "$tmp_file" "/app/data/prompts"; then
    alloyrag_mounts+=("./data/prompts:/app/data/prompts")
  fi

  append_alloyrag_ssl_mount alloyrag_mounts "${COMPOSE_ENV_OVERRIDES[SSL_CERTFILE]:-}" || return 1
  append_alloyrag_ssl_mount alloyrag_mounts "${COMPOSE_ENV_OVERRIDES[SSL_KEYFILE]:-}" || return 1
  if ((${#alloyrag_mounts[@]} > 0)); then
    inject_alloyrag_bind_mounts "$tmp_file" "${alloyrag_mounts[@]}"
  fi

  for key in "${!COMPOSE_ENV_OVERRIDES[@]}"; do
    alloyrag_env_entries+=("${key}=${COMPOSE_ENV_OVERRIDES[$key]}")
  done
  if ((${#alloyrag_env_entries[@]} > 0)); then
    inject_alloyrag_environment_overrides "$tmp_file" "${alloyrag_env_entries[@]}"
  fi

  repair_misplaced_alloyrag_depends_on "$tmp_file"
  inject_alloyrag_depends_on "$tmp_file" "${DOCKER_SERVICES[@]}"
...
prepare_alloyrag_service_for_generated_compose() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_alloyrag" == "yes" && "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ]]; then
      in_alloyrag="no"
    fi

    if [[ "$in_alloyrag" == "yes" && "$line" == "    env_file:" ]]; then
      in_env_file="yes"
      continue
    fi

    if [[ "$in_alloyrag" == "yes" && "$line" =~ ^[[:space:]]{4}container_name: ]]; then
      continue
    fi
...
normalize_alloyrag_restart_policy() {
  local compose_file="$1"
  local tmp_file="${compose_file}.normalize-alloyrag-restart"
  _FILE_OPS_CLEANUP_TMP+=("$tmp_file")
  local line
  local in_alloyrag="no"
  local in_deploy="no"
  ...
  _write_normalized_alloyrag_deploy_block() {
    local deploy_line
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_alloyrag" == "yes" && "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ]] || \
      [[ "$in_alloyrag" == "yes" && "$line" =~ ^[^[:space:]] ]]; then
    ...
    if [[ "$in_alloyrag" == "yes" && "$line" == "    deploy:" ]]; then
      in_deploy="yes"
      deploy_seen="yes"
      deploy_lines=()
      continue
    ...
    if [[ "$in_alloyrag" == "yes" && "$line" =~ ^[[:space:]]{4}restart: ]]; then
      skip_blank_after_removed_restart="yes"
      continue
    fi

    if [[ "$skip_blank_after_removed_restart" == "yes" && "$in_alloyrag" == "yes" ]]; then
      if [[ -z "$line" ]]; then
        continue
      fi
      skip_blank_after_removed_restart="no"
  ...
  if [[ "$in_alloyrag" == "yes" && "$deploy_seen" != "yes" ]]; then
    _trim_trailing_blank_lines "$tmp_file"
    _write_normalized_alloyrag_deploy_block
  fi
...
_COMPOSE_RAW_VALUE_PREFIX="__ALLOYRAG_RAW_COMPOSE__:"
...
# Return success when the alloyrag service already has a volume mount whose
...
_alloyrag_volumes_have_container_target() {
  local compose_file="$1"
  local target_path="$2"
  local line
  local in_alloyrag="no"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$in_alloyrag" == "yes" ]]; then
      if [[ "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ]] || \
         [[ "$line" =~ ^[^[:space:]] ]]; then
        in_alloyrag="no"
        in_volumes="no"
...
# Remove wizard-managed SSL bind mounts from the alloyrag service's volumes
# block, leaving persistent and user-added /app/data/* mounts intact.
_strip_alloyrag_wizard_bind_mounts() {
  local compose_file="$1"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$in_alloyrag" == "yes" ]]; then
      if [[ "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ]]; then
        in_alloyrag="no"
        in_volumes="no"
      elif [[ "$line" == "    volumes:" ]]; then
        in_volumes="yes"
...
_is_wizard_alloyrag_port_mapping() {
...
_strip_alloyrag_wizard_ports() {
  local compose_file="$1"
  local tmp_file="${compose_file}.strip-alloyrag-ports"
  _FILE_OPS_CLEANUP_TMP+=("$tmp_file")
  local line
  local port_spec=""
  local in_alloyrag="no"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$in_alloyrag" == "yes" && "$in_ports" == "yes" ]]; then
      if [[ "$line" =~ ^[[:space:]]{6}-[[:space:]](.+)$ ]]; then
        port_spec="${BASH_REMATCH[1]}"
        if _is_wizard_alloyrag_port_mapping "$port_spec"; then
          continue
        fi
      elif [[ ! "$line" =~ ^[[:space:]]{6} ]]; then
        in_ports="no"
    ...
    if [[ "$line" == "  alloyrag:" ]]; then
      in_alloyrag="yes"
      in_ports="no"
    elif [[ "$in_alloyrag" == "yes" && "$line" == "    ports:" ]]; then
      in_ports="yes"
    fi
  done < "$compose_file"
...
inject_alloyrag_bind_mounts() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$in_alloyrag" == "yes" && "$in_volumes" == "yes" ]]; then
    ...
    if [[ "$line" == "  alloyrag:" ]]; then
      in_alloyrag="yes"
      in_volumes="no"
    elif [[ "$in_alloyrag" == "yes" && "$line" == "    volumes:" ]]; then
      in_volumes="yes"
    fi
  done < "$compose_file"

  if [[ "$in_alloyrag" == "yes" && "$inserted" == "no" ]]; then
    if [[ "$in_volumes" != "yes" ]]; then
      printf '    volumes:\n' >> "$tmp_file"
    fi
    for mount in "${mounts[@]}"; do
...
repair_misplaced_alloyrag_depends_on() {
  local compose_file="$1"
  local tmp_file="${compose_file}.repair-alloyrag-depends-on"
  _FILE_OPS_CLEANUP_TMP+=("$tmp_file")
  local line
  local in_services="no"
  local in_alloyrag="no"
  local alloyrag_has_depends_on="no"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_alloyrag" == "yes" ]]; then
      if [[ "$line" == "    depends_on:" ]]; then
        alloyrag_has_depends_on="yes"
        break
      fi

      if [[ "$line" =~ ^[[:space:]]{2}([A-Za-z0-9_-]+):[[:space:]]*$ ]] && \
        [[ "${BASH_REMATCH[1]}" != "alloyrag" ]]; then
  ...
  if [[ "$alloyrag_has_depends_on" == "yes" || -z "$captured_block" || -z "$candidate_service" ]]; then
    return 0
  fi
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_alloyrag" == "yes" && "$inserted" == "no" ]] && \
      [[ ( "$line" =~ ^[[:space:]]{2}[^[:space:]] && "$line" != "  alloyrag:" ) || "$line" =~ ^[^[:space:]] ]]; then
...
inject_alloyrag_environment_overrides() {
  local compose_file="$1"
  shift
  inject_service_environment_overrides "$compose_file" "alloyrag" "$@"
}

inject_alloyrag_depends_on() {
  local compose_file="$1"
  shift
  local candidate_service
  ...
  _write_alloyrag_depends_on_block() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_alloyrag" == "yes" && "$line" == "    depends_on:" ]]; then
      in_depends_on="yes"
      continue
    fi
  ...
  elif [[ "$in_alloyrag" == "yes" && "$inserted" == "no" ]]; then
    _trim_trailing_blank_lines "$tmp_file"
    _write_alloyrag_depends_on_block
  fi
...
# Detect service names in a compose file's services: block (excluding alloyrag).
