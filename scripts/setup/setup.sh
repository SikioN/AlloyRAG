...
SSL_KEY_SOURCE_PATH=""
ALLOYRAG_COMPOSE_SERVER_PORT_MAPPING=""
...
PRESET_VLLM_RERANKER=(
  ...
  "ALLOYRAG_SETUP_RERANK_PROVIDER=vllm"
...
COMPOSE_ALLOYRAG_WORKING_DIR="/app/data/rag_storage"
COMPOSE_ALLOYRAG_INPUT_DIR="/app/data/inputs"
COMPOSE_ALLOYRAG_PROMPT_DIR="/app/data/prompts"
# shellcheck disable=SC2034
COLOR_RESET=""
COLOR_BOLD=""
COLOR_BLUE=""
...
validate_runtime_target() {
  ...
  case "$runtime_target" in
    ...
    *)
      format_error \
        "Invalid ALLOYRAG_RUNTIME_TARGET: ${runtime_target}" \
...
set_runtime_target() {
  ...
  ENV_VALUES["ALLOYRAG_RUNTIME_TARGET"]="$runtime_target"
}
...
normalize_vllm_rerank_binding_state() {
  if [[ "${ENV_VALUES[ALLOYRAG_SETUP_RERANK_PROVIDER]:-}" == "vllm" ]]; then
...
warn_if_network_exposed_without_auth() {
  ...
  if [[ -n "${ENV_VALUES[AUTH_ACCOUNTS]:-}" || -n "${ENV_VALUES[ALLOYRAG_API_KEY]:-}" ]]; then
    return 0
  fi

  log_warn "HOST=$host is reachable from the network but no authentication is configured."
  echo "  Suggestion: set AUTH_ACCOUNTS (with TOKEN_SECRET) or ALLOYRAG_API_KEY before"
...
normalize_server_host_for_compose() {
  ...
  ALLOYRAG_COMPOSE_SERVER_PORT_MAPPING='${HOST:-0.0.0.0}:${PORT:-9621}:9621'
...
prepare_compose_runtime_overrides() {
  ...
  # EMBEDDING_BINDING_HOST: when vllm-embed is part of this compose, the AlloyRAG
...
prepare_compose_data_path_overrides() {
  ...
  # Force alloyrag to use them so values from the mounted .env cannot redirect
  # storage into a different location.
  set_compose_override "WORKING_DIR" "$COMPOSE_ALLOYRAG_WORKING_DIR"
  set_compose_override "INPUT_DIR" "$COMPOSE_ALLOYRAG_INPUT_DIR"
  set_compose_override "PROMPT_DIR" "$COMPOSE_ALLOYRAG_PROMPT_DIR"
}
...
restore_vllm_docker_services_from_env() {
  if [[ "${ENV_VALUES[ALLOYRAG_SETUP_EMBEDDING_PROVIDER]:-}" == "vllm" ]]; then
...
resolve_compose_output_action() {
  ...
  local current_target="${ENV_VALUES[ALLOYRAG_RUNTIME_TARGET]:-$DEFAULT_RUNTIME_TARGET}"
  ...
  if [[ -n "$existing_compose" ]]; then
    if confirm_default_no "All wizard-managed services have been removed. Remove AlloyRAG from Docker and switch to host mode?"; then
  ...
  if [[ "$current_target" == "compose" ]]; then
    if confirm_default_yes "Run AlloyRAG Server via Docker?"; then
      action_ref="rewrite_compose"
      runtime_target_ref="compose"
    else
      host_hint_ref="yes"
    ...
    if confirm_default_no "Run AlloyRAG Server via Docker?"; then
      action_ref="rewrite_compose"
      runtime_target_ref="compose"
    else
      host_hint_ref="yes"
...
mongodb_service_requires_atlas_local_rewrite() {
  ...
  if [[ "${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-}" != "MongoVectorDBStorage" ]]; then
    return 1
  fi

  if [[ "${ENV_VALUES[ALLOYRAG_SETUP_MONGODB_DEPLOYMENT]:-}" != "docker" ]]; then
    return 1
  fi
...
env_value_changed_from_original() {
  local key="$1"
  local missing_marker="__ALLOYRAG_MISSING__"
...
select_storage_backends() {
  ...
  kv_default="${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-$kv_default}"
  vector_default="${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-$vector_default}"
  graph_default="${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]:-$graph_default}"
  doc_default="${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]:-$doc_default}"
  ...
  ENV_VALUES["ALLOYRAG_KV_STORAGE"]="$kv_storage"
  ENV_VALUES["ALLOYRAG_VECTOR_STORAGE"]="$vector_storage"
  ENV_VALUES["ALLOYRAG_GRAPH_STORAGE"]="$graph_storage"
  ENV_VALUES["ALLOYRAG_DOC_STATUS_STORAGE"]="$doc_storage"
...
initialize_default_storage_backends() {
  ...
  ENV_VALUES["ALLOYRAG_KV_STORAGE"]="${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-JsonKVStorage}"
  ENV_VALUES["ALLOYRAG_VECTOR_STORAGE"]="${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-NanoVectorDBStorage}"
  ENV_VALUES["ALLOYRAG_GRAPH_STORAGE"]="${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]:-NetworkXStorage}"
  ENV_VALUES["ALLOYRAG_DOC_STATUS_STORAGE"]="${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]:-JsonDocStatusStorage}"
}
...
storage_deployment_marker_key() {
  ...
  case "$db_type" in
    postgresql)
      printf 'ALLOYRAG_SETUP_POSTGRES_DEPLOYMENT'
      ;;
    neo4j)
      printf 'ALLOYRAG_SETUP_NEO4J_DEPLOYMENT'
      ;;
    mongodb)
      printf 'ALLOYRAG_SETUP_MONGODB_DEPLOYMENT'
      ;;
    redis)
      printf 'ALLOYRAG_SETUP_REDIS_DEPLOYMENT'
      ;;
    milvus)
      printf 'ALLOYRAG_SETUP_MILVUS_DEPLOYMENT'
      ;;
    qdrant)
      printf 'ALLOYRAG_SETUP_QDRANT_DEPLOYMENT'
      ;;
    memgraph)
      printf 'ALLOYRAG_SETUP_MEMGRAPH_DEPLOYMENT'
      ;;
    opensearch)
      printf 'ALLOYRAG_SETUP_OPENSEARCH_DEPLOYMENT'
      ;;
    *)
      printf ''
      ;;
...
collect_postgres_config() {
  ...
  # and host deployments share the same prompts and defaults (rag/rag/alloyrag).
  ...
  database="$(prompt_with_default "PostgreSQL database" "${existing_database:-alloyrag}")"
...
collect_mongodb_config() {
  ...
  database="$(prompt_with_default "MongoDB database" "${existing_database:-AlloyRAG}")"
...
collect_milvus_config() {
  ...
  db_name="$(prompt_with_default "Milvus database name" "${existing_db_name:-alloyrag}")"
...
collect_opensearch_config() {
  ...
  password="$(prompt_secret_until_valid_with_default "OpenSearch password: " "${existing_password:-AlloyRAG2026_!@}" validate_opensearch_password_strength)"
...
collect_embedding_config() {
  ...
  unset 'ENV_VALUES[ALLOYRAG_SETUP_EMBEDDING_PROVIDER]'
}
...
collect_rerank_config() {
  ...
  local previous_provider="${ENV_VALUES[ALLOYRAG_SETUP_RERANK_PROVIDER]:-}"
  ...
  if [[ "$skip_enable_check" != "yes" ]]; then
    ...
    if [[ "$rerank_enabled" != "yes" ]]; then
      ...
      unset 'ENV_VALUES[ALLOYRAG_SETUP_RERANK_PROVIDER]'
      return
    fi
  fi
...
collect_security_config() {
  ...
  api_key="$(prompt_clearable_secret_with_default "AlloyRAG API key: " "${ENV_VALUES[ALLOYRAG_API_KEY]:-}")"
  ...
  apply_clearable_env_value "ALLOYRAG_API_KEY" "$api_key"
...
prepare_inherited_ssl_assets_for_compose() {
  ...
  if [[ -n "$SSL_CERT_SOURCE_PATH" ]] && ! validate_existing_file "$SSL_CERT_SOURCE_PATH"; then
    if [[ -n "$existing_compose" ]]; then
      preserved_cert_path="$(read_service_environment_value "$existing_compose" "alloyrag" "SSL_CERTFILE" || true)"
    fi
  ...
  if [[ -n "$SSL_KEY_SOURCE_PATH" ]] && ! validate_existing_file "$SSL_KEY_SOURCE_PATH"; then
    if [[ -n "$existing_compose" ]]; then
      preserved_key_path="$(read_service_environment_value "$existing_compose" "alloyrag" "SSL_KEYFILE" || true)"
    fi
...
env_base_flow() {
  ...
  previous_embedding_provider="${ENV_VALUES[ALLOYRAG_SETUP_EMBEDDING_PROVIDER]:-}"
  ...
  if [[ "$use_docker_embed" == "yes" ]]; then
    ...
    ENV_VALUES["ALLOYRAG_SETUP_EMBEDDING_PROVIDER"]="vllm"
  ...
  previous_rerank_provider="${ENV_VALUES[ALLOYRAG_SETUP_RERANK_PROVIDER]:-}"
  ...
  if [[ "$enable_reranking" == "yes" ]]; then
    ...
    if [[ "$use_docker_rerank" == "yes" ]]; then
      ...
      ENV_VALUES["ALLOYRAG_SETUP_RERANK_PROVIDER"]="vllm"
...
finalize_base_setup() {
  ...
  if ! validate_mongo_vector_storage_config \
    "${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-}" \
    "${ENV_VALUES[MONGO_URI]:-}" \
    "${ENV_VALUES[ALLOYRAG_SETUP_MONGODB_DEPLOYMENT]:-}"; then
    return 1
  fi
  ...
  if ((${#DOCKER_SERVICES[@]} > 0)); then
    # AlloyRAG depends on managed Docker services; it must run via Docker.
    svc_names="$(printf '%s ' "${DOCKER_SERVICES[@]}")"
    svc_names="${svc_names% }"
    echo "AlloyRAG requires Docker services: ${svc_names}"
...
env_storage_flow() {
  ...
  log_debug "Storage selections: kv=${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-} vector=${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-} graph=${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]:-} doc=${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]:-}"
  clear_unused_storage_deployment_markers
...
finalize_storage_setup() {
  ...
  if [[ -n "${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-}" ]]; then
    if ! validate_required_variables \
      "${ENV_VALUES[ALLOYRAG_KV_STORAGE]}" \
      "${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]}" \
      "${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]}" \
      "${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]}"; then
      return 1
    fi
  fi
...
finalize_server_setup() {
  ...
  case "$compose_action" in
    rewrite_compose)
      ...
      echo "  To restart: docker compose -f ${compose_file} up -d --force-recreate alloyrag"
      ;;
    delete_compose_and_switch_host)
      remove_existing_compose_file "$existing_compose" || return 1
      echo "  To start: alloyrag-server"
...
validate_ssl_runtime_path() {
  local path="$1"
  local runtime_target="${ENV_VALUES[ALLOYRAG_RUNTIME_TARGET]:-$DEFAULT_RUNTIME_TARGET}"
  local staged_path=""
...
validate_env_file() {
  ...
  kv="${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-}"
  vector="${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-}"
  graph="${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]:-}"
  doc_status="${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]:-}"
  runtime_target="${ENV_VALUES[ALLOYRAG_RUNTIME_TARGET]:-$DEFAULT_RUNTIME_TARGET}"
  ...
  if [[ -z "$kv" || -z "$vector" || -z "$graph" || -z "$doc_status" ]]; then
    format_error "Storage selections are missing in .env" "Set ALLOYRAG_*_STORAGE variables."
    return 1
  fi
  ...
  if [[ -n "${referenced_db_types[opensearch]+set}" ]]; then
    if ! validate_opensearch_config \
      "${ENV_VALUES[ALLOYRAG_SETUP_OPENSEARCH_DEPLOYMENT]:-}" \
...
security_check_env_file() {
  ...
  api_key="${ENV_VALUES[ALLOYRAG_API_KEY]:-}"
  kv="${ENV_VALUES[ALLOYRAG_KV_STORAGE]:-}"
  vector="${ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]:-}"
  graph="${ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]:-}"
  doc_status="${ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]:-}"
  ...
  if [[ -n "${referenced_db_types[opensearch]+set}" || "${ENV_VALUES[ALLOYRAG_SETUP_OPENSEARCH_DEPLOYMENT]:-}" == "docker" ]]; then
  ...
  if [[ -z "$auth_accounts" && -z "$api_key" ]]; then
    ...
    report_security_issue \
      "$no_auth_message" \
      "Set AUTH_ACCOUNTS and TOKEN_SECRET, add ALLOYRAG_API_KEY, or put the service behind a trusted reverse proxy."
    findings=$((findings + 1))
  fi
  ...
  if [[ -n "$auth_accounts" ]]; then
    if ! validate_auth_accounts_format "$auth_accounts"; then
      ...
      report_security_issue \
        ...
        "Passwords must not start with 'admin' or 'pass'. Choose a stronger password or use alloyrag-hash-password."
      findings=$((findings + 1))
    fi
    ...
    if [[ -z "$token_secret" ]]; then
      ...
      findings=$((findings + 1))
    elif [[ "$token_secret" == "alloyrag-jwt-default-secret" ]]; then
      report_security_issue \
  ...
  if [[ -z "$auth_accounts" && -n "$api_key" ]]; then
    ...
    if whitelist_exposes_api_routes "$effective_whitelist"; then
      report_security_issue \
        "WHITELIST_PATHS exposes /api routes while ALLOYRAG_API_KEY is the only active auth mechanism." \
  ...
  if [[ "$opensearch_in_use" == "yes" ]] && [[ -n "${ENV_VALUES[OPENSEARCH_PASSWORD]:-}" ]]; then
    ...
    if [[ "$os_pass" == "admin" || "$os_pass" == "AlloyRAG2026_!@" ]]; then
      report_security_issue \
