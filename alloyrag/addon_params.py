...
``addon_params`` is a free-form configuration dict on :class:`AlloyRAG` that
...
- :class:`ObservableAddonParams` — a ``dict`` subclass that calls a callback
  whenever the contents change so the AlloyRAG runtime can invalidate cached
  derived state.
...
from alloyrag.constants import DEFAULT_SUMMARY_LANGUAGE
...
def default_addon_params() -> dict[str, Any]:
    ...
    # module via ``AlloyRAG`` construction paths.
    from alloyrag.parser.routing import default_chunker_config
