...
received. When a document was parsed into the AlloyRAG sidecar format
...
:func:`alloyrag.sidecar.writer.write_sidecar` and
:func:`alloyrag.utils_pipeline.load_alloyrag_document_content` build it as
...
from alloyrag.chunk_schema import normalize_chunk_sidecar
from alloyrag.exceptions import ChunkBlockMatchError
from alloyrag.utils import logger

# Separator used to join block content into the merged document text. Must match
# alloyrag/sidecar/writer.py and alloyrag/utils_pipeline.py.
_BLOCK_SEPARATOR = "\n\n"
...
def _load_content_blocks(blocks_path: str) -> list[tuple[str, str]]:
    ...
    Note: ``utils_pipeline.load_alloyrag_document_content`` (which produces the
...
def _build_block_spans(
    ...
    ``utils_pipeline.load_alloyrag_document_content``). Returns ``(merged, spans)``
