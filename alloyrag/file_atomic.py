...
Why this lives at the package root rather than under ``alloyrag/kg/``:
``alloyrag.utils.write_json`` needs ``atomic_write`` to gain crash safety,
and several ``alloyrag/kg/*`` modules need both. Hosting the helpers under
``alloyrag/kg/`` would create a ``utils -> kg -> utils`` import cycle.
