    ...
    try:
        with open(filepath, "rb") as f:
            ...
            if response.status_code == 200:
                ...
                markdown = doc.get("md_content") or doc.get("text") or res_json.get("text")
                if markdown:
                    logger.info("Successfully converted PDF to Markdown via Docling.")
                    return markdown
            logger.warning(f"Docling Serve returned code {response.status_code}: {response.text}")
    except Exception as e:
...
async def aingest_file(filepath: str) -> bool:
    ...
    if ext in [".xlsx", ".xls"]:
        ...
        logger.info(f"Ingesting {len(custom_kg['entities'])} entities and {len(custom_kg['relationships'])} relations asynchronously into AlloyRAG...")
        try:
    ...
    if ext == ".pdf":
        ...
        if not content:
            logger.warning("Docling parser unavailable or failed. Falling back to local pypdf extraction.")
...
async def ascan_and_ingest_inputs():
    ...
    for filepath in files_in_dir:
        ...
        if state and state.get("md5") == current_md5 and state.get("status") == "success":
