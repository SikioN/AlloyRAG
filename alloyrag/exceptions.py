...
class PipelineNotInitializedError(KeyError):
    ...
    def __init__(self, namespace: str = ""):
        msg = (
            ...
            f"  2. For multi-workspace setups, each AlloyRAG instance was properly initialized\n"
            f"\n"
            f"Standard initialization:\n"
            f"  rag = AlloyRAG(workspace='your_workspace')\n"
            f"  await rag.initialize_storages()  # Auto-initializes pipeline_status\n"
            f"\n"
            f"If you need manual control (advanced):\n"
            f"  from alloyrag.kg.shared_storage import initialize_pipeline_status\n"
...
class ChunkBlockMatchError(ValueError):
    ...
    Sidecar backfill (``alloyrag.sidecar.backfill``) maps F/R/V chunks back to
