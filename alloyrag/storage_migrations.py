"""Storage data migration helpers for :class:`AlloyRAG`.

Mixed into AlloyRAG and runs once at startup (``initialize_storages`` →
...
from alloyrag.kg.shared_storage import get_data_init_lock
from alloyrag.utils import logger, make_relation_chunk_key


class _StorageMigrationMixin:
    """Mixin that owns one-shot data migrations on :class:`AlloyRAG`.

    Mixed into AlloyRAG only. Relies on attributes that the main class
