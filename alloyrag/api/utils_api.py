"""
Utility functions for the AlloyRAG API.
"""
...
from alloyrag.api.runtime_validation import validate_runtime_target_from_env_file
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing AlloyRAG server configuration
    ...
    line1_text = f"AlloyRAG Server v{core_version}/{api_version}"
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.ALLOYRAG_MODEL}")
    ...
    # neither AUTH_ACCOUNTS nor ALLOYRAG_API_KEY set, all requests are allowed).
    ...
    reachable from this machine. Set ALLOYRAG_API_KEY, or AUTH_ACCOUNTS together
    ...
    Secure the server before exposing it to a network by setting at least one of:
      - ALLOYRAG_API_KEY=<a-strong-secret>   (X-API-Key header authentication)
