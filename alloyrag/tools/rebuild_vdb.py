#!/usr/bin/env python3
"""
Offline Vector Storage (VDB) Rebuild Tool for AlloyRAG

The knowledge graph and the text_chunks KV store are the authoritative data
sources in AlloyRAG. If a vector storage write fails at runtime (e.g. during
...
IMPORTANT: Shut down the AlloyRAG Server (and any other writers) before
running this tool.

Usage:
    alloyrag-rebuild-vdb
    # or
    python -m alloyrag.tools.rebuild_vdb

Configuration is read from .env / environment variables, exactly like the
AlloyRAG server (ALLOYRAG_GRAPH_STORAGE, ALLOYRAG_VECTOR_STORAGE,
ALLOYRAG_KV_STORAGE, WORKSPACE, WORKING_DIR, EMBEDDING_* ...). The embedding
...
from alloyrag.kg import STORAGE_ENV_REQUIREMENTS
...
async def rebuild_entities_vdb(
    ...
    *,
    ...
    """
    from alloyrag.operate import _truncate_vdb_content
...
async def enumerate_kv_keys(kv) -> List[str]:
    ...
    scans (same patterns as alloyrag/tools/clean_llm_query_cache.py).
    ...
    if storage_name == "PGKVStorage":
        from alloyrag.kg.postgres_impl import namespace_to_table_name
...
class RebuildTool:
    ...
    def resolve_storage_names(self) -> Dict[str, str]:
        return {
            "graph": os.getenv("ALLOYRAG_GRAPH_STORAGE", "NetworkXStorage"),
            "vector": os.getenv("ALLOYRAG_VECTOR_STORAGE", "NanoVectorDBStorage"),
            "kv": os.getenv("ALLOYRAG_KV_STORAGE", "JsonKVStorage"),
        }
    ...
    def build_embedding_func(self) -> EmbeddingFunc | None:
        ...
        """
        try:
            from alloyrag.api.config import global_args
            from alloyrag.api.alloyrag_server import (
                create_embedding_function_from_args,
            )
        except ImportError as e:
            print(f"\n⚠️  Could not import the AlloyRAG API package: {e}")
            print('   Rebuild requires the api extra: pip install "alloyrag-hku[api]"')
    ...
    def build_global_config(self) -> Dict[str, Any]:
        global_config: Dict[str, Any] = {
            ...
            # Backend selection, mirroring AlloyRAG._build_global_config. PG
        ...
        if max_token_size:
            try:
                from alloyrag.utils import TiktokenTokenizer
    ...
    async def setup_storages(self) -> bool:
        ...
        if not self.embedding_available:
            ...
            async def _no_embedding(*_args, **_kwargs):
                raise RuntimeError(
                    ...
                    'Install the api extra: pip install "alloyrag-hku[api]"'
                )
        ...
        # Namespaces and meta_fields must match AlloyRAG's own storage setup
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            from alloyrag.kg.shared_storage import initialize_share_data
            ...
            for storage in self.all_storages():
                if storage is not None:
                    try:
                        ...
                        pass
            try:
                from alloyrag.kg.shared_storage import finalize_share_data
