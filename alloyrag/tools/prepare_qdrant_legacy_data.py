#!/usr/bin/env python3
"""
Qdrant Legacy Data Preparation Tool for AlloyRAG
...
New Collections (with workspace_id):
    - alloyrag_vdb_chunks
    - alloyrag_vdb_entities
    - alloyrag_vdb_relationships
...
Usage:
    python -m alloyrag.tools.prepare_qdrant_legacy_data
    # or
    python alloyrag/tools/prepare_qdrant_legacy_data.py

    # Specify custom workspace
    python -m alloyrag.tools.prepare_qdrant_legacy_data --workspace space1

    # Process specific collection types only
    python -m alloyrag.tools.prepare_qdrant_legacy_data --types chunks,entities

    # Dry run (preview only, no actual changes)
    python -m alloyrag.tools.prepare_qdrant_legacy_data --dry-run
"""
...
COLLECTION_NAMESPACES = {
    "chunks": {
        "new": "alloyrag_vdb_chunks",
        "suffix": "chunks",
    },
    "entities": {
        "new": "alloyrag_vdb_entities",
        "suffix": "entities",
    },
    "relationships": {
        "new": "alloyrag_vdb_relationships",
...
class QdrantLegacyDataPreparationTool:
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("Qdrant Legacy Data Preparation Tool - AlloyRAG")
        print("=" * 60)
        if self.dry_run:
