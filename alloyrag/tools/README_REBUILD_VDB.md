...
`alloyrag-rebuild-vdb` restores consistency between AlloyRAG's authoritative
...
AlloyRAG performs multi-step writes (graph + vector storage). If a vector
...
```bash
# Stop the AlloyRAG Server first!
...
(`ALLOYRAG_GRAPH_STORAGE`, `ALLOYRAG_VECTOR_STORAGE`, `ALLOYRAG_KV_STORAGE`,
