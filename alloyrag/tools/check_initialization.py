#!/usr/bin/env python3
"""
Diagnostic tool to check AlloyRAG initialization status.

This tool helps developers verify that their AlloyRAG instance is properly
...
Usage:
    ...
    await check_alloyrag_setup(rag, verbose=True)

    # Run demo from command line:
    python -m alloyrag.tools.check_initialization --demo
"""
...
from alloyrag.base import StoragesStatus


async def check_alloyrag_setup(rag_instance: AlloyRAG, verbose: bool = False) -> bool:
    """
    Check if a AlloyRAG instance is properly initialized.

    Args:
        rag_instance: The AlloyRAG instance to check
    ...
    print("🔍 Checking AlloyRAG initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("AlloyRAG instance missing _storages_status attribute")
    ...
    try:
        from alloyrag.kg.shared_storage import get_namespace_data
    ...
    if issues:
        ...
        print("✅ AlloyRAG is properly initialized and ready to use!")
...
async def demo():
    ...
    from alloyrag.llm.openai import openai_embed, gpt_4o_mini_complete

    print("=" * 50)
    print("AlloyRAG Initialization Diagnostic Tool")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check AlloyRAG initialization status")
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Or import this module and use check_alloyrag_setup() with your instance")
