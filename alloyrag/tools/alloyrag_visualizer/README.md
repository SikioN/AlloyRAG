# AlloyRAG 3D Graph Viewer

An interactive 3D graph visualization tool included in the AlloyRAG package for visualizing and analyzing RAG (Retrieval-Augmented Generation) graphs and other graph structures.
...
### Quick Install
```bash
pip install alloyrag-hku[tools]  # Install with visualization tool only
# or
pip install alloyrag-hku[api,tools]  # Install with both API and visualization tools
```
...
1. **Command Not Found**
   ...
   pip install alloyrag-hku[tools]

   # Verify installation
   pip list | grep alloyrag-hku
   ```
...
This tool is part of AlloyRAG and is distributed under the MIT License. See `LICENSE` for more information.

Note: This visualization tool is an optional component of the AlloyRAG package. Install with the [tools] option to access the viewer functionality.
