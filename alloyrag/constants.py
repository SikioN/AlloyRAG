"""
Centralized configuration constants for AlloyRAG.

This module defines default values for configuration constants used across
different parts of the AlloyRAG system. Centralizing these values ensures
...
# entity_name / src_id / tgt_id fields in alloyrag/kg/milvus_impl.py.
...
# enqueue docs_format (the 'alloyrag' ingestion entrypoint was removed).
FULL_DOCS_FORMAT_ALLOYRAG = "alloyrag"  # content in AlloyRAG Document files
FULL_DOCS_FORMAT_PENDING_PARSE = (
    "pending_parse"  # file saved but not yet parsed; parse_native will read from disk
)
# Marker prefix for full_docs.content when format=alloyrag.
...
# preview without loading the full AlloyRAG Document file.
ALLOYRAG_DOC_CONTENT_PREFIX = "{{LRdoc}}"
# Engine identifier strings (registry keys). The set of user-selectable
# engines and their suffix capabilities now live in
# alloyrag.parser.registry (ParserSpec table) — the single source of truth.
...
# Per-file processing options carried by filename hints / ALLOYRAG_PARSER rules.
