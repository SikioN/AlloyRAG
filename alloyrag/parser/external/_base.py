...
``ExternalParserBase.parse`` fixes the common MinerU/Docling flow once:
    ...
    → write_sidecar → persist full_docs (alloyrag) → archive source
...
from alloyrag.parser.base import BaseParser, ParseContext, ParseResult
...
class ExternalParserBase(BaseParser):
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        from alloyrag.parser.external._common import (
            clear_dir_contents,
            env_bool,
            raw_dir_for_parsed_dir,
        )
        from alloyrag.parser.routing import decode_parse_engine, encode_parse_engine
        from alloyrag.sidecar import write_sidecar
        ...
        await ctx.rag._persist_parsed_full_docs(
            ctx.doc_id,
            {
                "content": make_alloyrag_doc_content(parsed_data["content"]),
