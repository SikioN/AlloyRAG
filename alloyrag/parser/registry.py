...
Like :data:`alloyrag.kg.STORAGES` / ``STORAGE_IMPLEMENTATIONS``, this module
...
if TYPE_CHECKING:
    from alloyrag.parser.base import BaseParser
...
class ParserSpec:
    ...
    # mirroring how the built-in ``max_parallel_parse_*`` AlloyRAG fields read
    # their env. ``None`` means this spec does not own its group's concurrency
    # (built-in groups are sized by the AlloyRAG instance field instead).
...
_REGISTRY: dict[str, ParserSpec] = {
    PARSER_ENGINE_NATIVE: ParserSpec(
        ...
        # source suffix (see alloyrag.parser.native_dispatch).
        impl="alloyrag.parser.native_dispatch:NativeParser",
        suffixes=frozenset({"docx", "md", "textpack"}),
        queue_group=PARSER_ENGINE_NATIVE,
        # Built-in groups are sized by the AlloyRAG ``max_parallel_parse_*``
    ...
    PARSER_ENGINE_LEGACY: ParserSpec(
        ...
        impl="alloyrag.parser.legacy.parser:LegacyParser",
    ...
    PARSER_ENGINE_MINERU: ParserSpec(
        ...
        impl="alloyrag.parser.external.mineru.parser:MinerUParser",
    ...
    PARSER_ENGINE_DOCLING: ParserSpec(
        ...
        impl="alloyrag.parser.external.docling.parser:DoclingParser",
    ...
    PARSER_ENGINE_REUSE: ParserSpec(
        ...
        impl="alloyrag.parser.noop:ReuseParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
    PARSER_ENGINE_PASSTHROUGH: ParserSpec(
        engine_name=PARSER_ENGINE_PASSTHROUGH,
        impl="alloyrag.parser.noop:PassthroughParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
}
