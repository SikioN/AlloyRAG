...
(:mod:`alloyrag.parser.registry`) instead of a growing ``if engine == …``
chain.
...
if TYPE_CHECKING:
    from alloyrag.sidecar.ir import IRDoc  # noqa: F401
...
class ParseContext:
    ...
    Wraps the AlloyRAG instance plus the per-document handles so a parser
    ...
    async def archive_source(self, source_path: str) -> str | None:
        ...
        ``alloyrag.pipeline.archive_docx_source_after_full_docs_sync`` keep
