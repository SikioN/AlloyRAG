...
from alloyrag.parser.param_schema import (
    parse_chunk_params,
    parse_engine_params,
...
from alloyrag.utils import logger, parse_optional_float
...
# there automatically (no filename hint / ALLOYRAG_PARSER rule needed). ``.md``
...
class ParserRoutingConfigError(ValueError):
    """Raised when ALLOYRAG_PARSER contains an invalid routing rule."""
...
class ProcessOptions:
    ...
    def chunking_explicit(self) -> bool:
        ...
        legacy externally-supplied :attr:`AlloyRAG.chunking_func`.
        """
...
def default_chunker_config() -> dict[str, Any]:
    ...
    *strategy-specific* env vars (``CHUNK_F_SIZE``,
    ...
    :meth:`AlloyRAG._apply_chunk_size_overlay` apply the legacy
    constructor field (``AlloyRAG(chunk_overlap_token_size=…)``) and
...
def resolve_chunk_options(
    ...
    *,
    ...
    The F runtime args from ``AlloyRAG.ainsert`` overlay the
...
def parser_rules_from_env() -> str:
    return os.getenv("ALLOYRAG_PARSER", "").strip()
...
def _rule_engine_and_options(engine_hint: str) -> tuple[str, str]:
    """Split a ``ALLOYRAG_PARSER`` rule's RHS (``engine[-options]``).
    ...
    boundary, since ``ALLOYRAG_PARSER`` rules cannot be options-only.
    """
...
def validate_parser_routing_config(parser_rules: str | None = None) -> None:
    """Validate ALLOYRAG_PARSER syntax and required external parser endpoints."""
    ...
    if errors:
        raise ParserRoutingConfigError(
            "Invalid ALLOYRAG_PARSER configuration: " + "; ".join(errors)
        )
...
def _matching_rule_directives(
    ...
    *,
    ...
    """Find the first matching ``ALLOYRAG_PARSER`` rule for ``file_path``.
...
def resolve_parser_directives(
    ...
    *,
    ...
    Resolution order (mirrors :func:`resolve_file_parser_engine`):
        ...
        2. ``ALLOYRAG_PARSER`` rules — first matching rule provides defaults
...
def resolve_stored_document_parser_engine(
    ...
    if content_data:
        ...
        # All alloyrag rows reuse the already-parsed sidecar (sidecar optional;
        # ReuseParser tolerates a missing one). Reached on resume/retry.
        if doc_format == FULL_DOCS_FORMAT_ALLOYRAG:
