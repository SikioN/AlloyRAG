"""Shared debug AlloyRAG stand-in for registry-dispatched parsing.

A minimal ``AlloyRAG`` stand-in plus a deterministic ``datetime`` shim,
shared by the unified parser debug CLI (``alloyrag/parser/cli.py``),
...
def build_debug_rag():
    """Build a minimal AlloyRAG stand-in that exposes what a parser reads.

    The import of ``AlloyRAG`` is intentionally function-local: deferring
    it avoids a circular import when this helper is loaded during package
    init (the parser CLI resolves ``alloyrag.parser.debug`` before
    ``alloyrag`` itself is fully bound).
    ...
    - **methods** (rebound from :class:`AlloyRAG`):
