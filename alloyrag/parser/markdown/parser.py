...
from alloyrag.constants import NATIVE_RAW_DIR_SUFFIX, PARSER_ENGINE_NATIVE
from alloyrag.parser.external._common import raw_dir_for_parsed_dir
from alloyrag.parser.markdown.extract import ResolvedImage, extract_markdown
from alloyrag.parser.markdown.raw_cache import (
    NativeImageRawCache,
    native_md_options_signature,
)
from alloyrag.parser.native_base import NativeParserBase
from alloyrag.utils import logger

if TYPE_CHECKING:
    from alloyrag.sidecar.ir import IRDoc
...
class _MarkdownImageResolver:
    """Concrete :class:`~alloyrag.parser.markdown.extract.ImageResolver`.
    ...
    def _download(self, src: str) -> tuple[bytes, str]:
        ...
        req = urllib.request.Request(src, headers={"User-Agent": "alloyrag-native-md"})
...
class NativeMarkdownParser(NativeParserBase):
    ...
    def extract(
        ...
        raw_cache = NativeImageRawCache(
            ...
            force_reparse=_env_bool("ALLOYRAG_FORCE_REPARSE_NATIVE", False),
        )
        raw_cache.load()
    ...
    def _open_textpack(self, source: Path, tmp_dir: Path) -> tuple[str, Path]:
        from alloyrag.parser.external._zip import safe_extract_zip
    ...
    def build_ir(
        ...
        *,
        ...
        from alloyrag.parser.markdown.ir_builder import NativeMarkdownIRBuilder
