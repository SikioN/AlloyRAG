...
- :class:`ReuseParser` handles ``alloyrag`` rows — a document already parsed
...
from alloyrag.constants import FULL_DOCS_FORMAT_ALLOYRAG, FULL_DOCS_FORMAT_RAW
from alloyrag.parser.base import BaseParser, ParseContext, ParseResult


class ReuseParser(BaseParser):
    """Reuse an already-parsed (``alloyrag``-format) document's sidecar."""
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        ...
        doc_format = ctx.content_data.get("parse_format", FULL_DOCS_FORMAT_ALLOYRAG)
        merged_text = strip_alloyrag_doc_prefix(
