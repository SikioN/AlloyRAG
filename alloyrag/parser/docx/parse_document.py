...
from alloyrag.parser._markdown import (
    render_heading_line,
...
def _diagnose_invalid_docx(file_path: str) -> tuple[str, str]:
    ...
    convert_solution = (
        ...
        "  3. Re-upload the converted .docx to AlloyRAG"
    )
    ...
    if not head:
        return (
            ...
            "  2. Re-upload a complete copy to AlloyRAG",
        )
    ...
    if head.startswith(b"PK\x03\x04") and not zipfile.is_zipfile(file_path):
        ...
        return (
            ...
            "  1. Check the original document opens correctly\n"
            "  2. Re-upload a complete, uncorrupted copy to AlloyRAG",
        )
...
def extract_para_id(para_element) -> str:
    ...
    Returns:
        ...
        propagate the ``None`` upward — the AlloyRAG adapter counts these
...
def extract_docx_blocks(
    ...
    Args:
        ...
        parse_warnings: Optional out-dict that this function mutates with
            ...
            per table whose every cell lacks one. Callers (the AlloyRAG
        ...
        parse_metadata: Optional out-dict that this function mutates with
            ...
            in document order (regardless of level). Used by the AlloyRAG
