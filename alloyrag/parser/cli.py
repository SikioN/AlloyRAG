...
Invocation::
    ...
    python -m alloyrag.parser.cli path/to/sample.docx --engine native
    python -m alloyrag.parser.cli path/to/sample.pdf  --engine mineru
    python -m alloyrag.parser.cli path/to/sample.pdf  --engine docling --force-reparse
...
def _normalize_direct_script_sys_path() -> None:
    ...
    # Direct execution adds alloyrag/parser to sys.path, which makes the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    from alloyrag.parser.registry import supported_parser_engines
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from alloyrag.constants import FULL_DOCS_FORMAT_PENDING_PARSE
    from alloyrag.parser.base import ParseContext
    from alloyrag.parser.external._base import ExternalParserBase
    from alloyrag.parser.registry import get_parser, suffix_capabilities
    from alloyrag.parser.debug import build_debug_rag
    from alloyrag.parser.docx.parse_document import DocxContentError
    from alloyrag.utils import compute_mdhash_id
    import alloyrag.pipeline as pipeline_mod
    import alloyrag.utils_pipeline as utils_pipeline_mod
    ...
    with ExitStack() as stack:
        ...
        # ctx.archive_source -> alloyrag.pipeline.archive_docx_source_after_...
        stack.enter_context(
            mock.patch.object(
                pipeline_mod,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # so a `alloyrag.parsers` entry-point engine is selectable here exactly
