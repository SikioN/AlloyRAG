...
These helpers are shared by the AlloyRAG pipeline mixin (alloyrag/pipeline.py)
and by other AlloyRAG methods that touch the document ingestion paths
...
from alloyrag.base import DocProcessingStatus, DocStatus, DocStatusStorage
...
from alloyrag.parser.routing import canonicalize_parser_hinted_basename
...
def resolve_doc_status_parse_engine(
    ...
    structured ``alloyrag`` format and ``legacy`` for everything else
    ...
    return (
        PARSER_ENGINE_NATIVE
        if parse_format == FULL_DOCS_FORMAT_ALLOYRAG
...
def compute_file_content_hash(path_str: str) -> str | None:
    ...
    Resolves the AlloyRAG ``*.blocks.jsonl`` conventions used by
    ``_load_alloyrag_document_content`` so the hash matches the actual
...
def make_alloyrag_doc_content(merged_text: str) -> str:
    """Build the ``full_docs.content`` value for ``format=alloyrag`` records.

    The result has shape ``"{{LRdoc}}<merged_text>"`` — the marker prefix
    distinguishes alloyrag-format full_docs from raw-format ones, and the
    body is the complete merged text from the ``.blocks.jsonl`` content
    lines so F-chunking can run identically on raw and alloyrag inputs
    (the prefix is stripped at chunking time via
    ``strip_alloyrag_doc_prefix``).
    """
    return f"{ALLOYRAG_DOC_CONTENT_PREFIX}{merged_text or ''}"


def strip_alloyrag_doc_prefix(content: str | None, parse_format: str | None) -> str:
    """Return the bare body for a stored ``full_docs.content`` value.

    The ``{{LRdoc}}`` marker is stripped **only** when ``parse_format``
    indicates the record is in alloyrag format.  Any other ``parse_format``
    ...
    if (
        ...
        and content.startswith(ALLOYRAG_DOC_CONTENT_PREFIX)
    ):
        return content[len(ALLOYRAG_DOC_CONTENT_PREFIX) :]
    return content or ""
...
# ---------------------------------------------------------------------------
# AlloyRAG Document blocks loader
# ---------------------------------------------------------------------------


async def load_alloyrag_document_content(sidecar_uri: str) -> tuple[str, str]:
    """Load AlloyRAG Document blocks and return ``(merged_text, blocks_path)``.
    ...
    if resolved is None:
        raise FileNotFoundError(
            f"AlloyRAG blocks file not found from sidecar uri: {sidecar_uri}"
        )
...
# :func:`alloyrag.operate.extract_entities._process_single_content` so each
