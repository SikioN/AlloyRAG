...
### 📁 AlloyRAG Evalua'tion Framework Directory Structure

```
alloyrag/evaluation/
├── eval_rag_quality.py      # Main evaluation script
├── sample_dataset.json        # 3 test questions about AlloyRAG
├── sample_documents/          # Matching markdown files for testing
│   ├── 01_alloyrag_overview.md
│   ├── 02_rag_architecture.md
│   ├── 03_alloyrag_improvements.md
...
**Quick Test:** Index files from `sample_documents/` into AlloyRAG, then run the evaluator to reproduce results (~89-100% RAGAS score per question).
...
```bash
python alloyrag/evaluation/offline_retrieval_check.py --strict
```

This checks whether the bundled sample questions can lexically retrieve their
expected sample documents before running AlloyRAG, embeddings, LLM calls, or
RAGAS.

**Basic usage (uses defaults):**
```bash
cd /path/to/AlloyRAG
python alloyrag/evaluation/eval_rag_quality.py
```

**Specify custom dataset:**
```bash
python alloyrag/evaluation/eval_rag_quality.py --dataset my_test.json
```

**Specify custom RAG endpoint:**
```bash
python alloyrag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
```

**Specify both (short form):**
```bash
python alloyrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
```

**Get help:**
```bash
python alloyrag/evaluation/eval_rag_quality.py --help
```

### 3. View Results

Results are saved automatically in `alloyrag/evaluation/results/`:
...
| `--ragendpoint` | `-r` | `http://localhost:9621` or `$ALLOYRAG_API_URL` | AlloyRAG API endpoint URL |
...
```bash
python alloyrag/evaluation/eval_rag_quality.py --dataset path/to/my_dataset.json
```
...
```bash
python alloyrag/evaluation/eval_rag_quality.py -d my_dataset.json -r http://localhost:9621
```

**Absolute path to dataset:**
```bash
python alloyrag/evaluation/eval_rag_quality.py -d /path/to/custom_dataset.json
```
...
EVAL_QUERY_TOP_K=10      # OP_K query parameter of AlloyRAG
...
`sample_dataset.json` contains 3 generic questions about AlloyRAG. Replace with questions matching YOUR indexed documents.
...
### "AlloyRAG query API errors during evaluation"

The evaluation uses your configured LLM (OpenAI by default). Ensure:
- API keys are set in `.env`
- Network connection is stable

### Evaluation requires running AlloyRAG API

The evaluator queries a running AlloyRAG API server at `http://localhost:9621`. Make sure:
1. AlloyRAG API server is running (`python alloyrag/api/alloyrag_server.py`)
2. Documents are indexed in your AlloyRAG instance
...
1. Start AlloyRAG API server
2. Upload sample documents into AlloyRAG  throught  WebUI
3. Run `python alloyrag/evaluation/eval_rag_quality.py`
...
```
INFO: ======================================================================
INFO: 🔍 RAGAS Evaluation - Using Real AlloyRAG API
INFO: ======================================================================
...
INFO:   • AlloyRAG API:         http://localhost:9621
INFO:   • Results Directory:    results
INFO: ======================================================================
INFO: 🚀 Starting RAGAS Evaluation of AlloyRAG System
...
INFO: -------------------------------------------------------------------------------------------------------------------
INFO: 1    | How does AlloyRAG solve the hallucination probl... | 1.0000 |  1.0000 | 1.0000 |  1.0000 | 1.0000 |      ✓
INFO: 2    | What are the three main components required in ... | 0.8500 |  0.5790 | 1.0000 |  1.0000 | 0.8573 |      ✓
INFO: 3    | How does AlloyRAG's retrieval performance compa... | 0.8056 |  1.0000 | 1.0000 |  1.0000 | 0.9514 |      ✓
INFO: 4    | What vector databases does AlloyRAG support and... | 0.8182 |  0.9807 | 1.0000 |  1.0000 | 0.9497 |      ✓
INFO: 5    | What are the four key metrics for evaluating RA... | 1.0000 |  0.7452 | 1.0000 |  1.0000 | 0.9363 |      ✓
INFO: 6    | What are the core benefits of AlloyRAG and how ... | 0.9583 |  0.8829 | 1.0000 |  1.0000 | 0.9603 |      ✓
INFO: ===================================================================================================================
INFO:
INFO: ======================================================================
