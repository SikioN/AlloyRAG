#!/usr/bin/env python3
"""Offline sanity check for the bundled AlloyRAG evaluation samples.

The check uses a small deterministic lexical ranker. It does not start
AlloyRAG, call the API server, compute embeddings, or call LLM/RAGAS services.
"""
...
def print_report(results: list[QueryResult], top_k: int) -> None:
    ...
    print("AlloyRAG sample retrieval check")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an offline retrieval check for AlloyRAG evaluation samples."
    )
