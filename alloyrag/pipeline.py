"""Document ingestion pipeline mixin for the AlloyRAG class.

This module isolates the document parse/enqueue/extraction pipeline so that
``alloyrag.py`` stays focused on storage management, querying, and editing.
The mixin is wired into :class:`alloyrag.AlloyRAG` via multiple inheritance
...
from alloyrag.base import DocProcessingStatus, DocStatus
from alloyrag.constants import (
    FULL_DOCS_FORMAT_ALLOYRAG,
    FULL_DOCS_FORMAT_PENDING_PARSE,
    FULL_DOCS_FORMAT_RAW,
    PARSED_DIR_NAME,
)
from alloyrag.exceptions import (
    MultimodalAnalysisError,
    PipelineCancelledException,
    IndexFlushError,
)
from alloyrag.kg.shared_storage import get_namespace_data, get_namespace_lock
from alloyrag.operate import merge_nodes_and_edges
from alloyrag.parser.base import ParseContext
from alloyrag.parser.registry import (
    get_parser,
...
from alloyrag.utils_pipeline import (
    # Re-exported through the pipeline namespace (not used by this module
    # directly): the parser layer resolves these as ``alloyrag.pipeline.<name>``
...
class _PipelineMixin:
    """Mixin providing document ingestion pipeline methods for AlloyRAG.

    Designed to be combined as a base of AlloyRAG only.  Relies on
    AlloyRAG-provided attributes (``self.full_docs``, ``self.doc_status``,
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated (from content or file_path).
                **Providing ``ids`` marks the SDK raw direct-insert path**
                (:meth:`AlloyRAG.ainsert`) and takes precedence over
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :func:`alloyrag.parser.routing.resolve_chunk_options`
                ...
                :meth:`AlloyRAG.ainsert` — should call
        ...
        if docs_format not in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_PENDING_PARSE):
            raise ValueError(
                ...
                "The 'alloyrag' enqueue format was removed; already-parsed "
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from alloyrag.parser.routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            F-strategy runtime args (``split_by_character`` /
            ``split_by_character_only`` from :meth:`AlloyRAG.ainsert`)
            are baked into the snapshot upstream — ainsert calls
            :func:`alloyrag.parser.routing.resolve_chunk_options` itself
        ...
        # 1. Validate ids and build contents (when alloyrag: no content dedup, content may be empty)
        if ids is not None:
    ...
    async def _run_pipeline_batch(
        ...
        *,
        ...
        def _group_concurrency(group: str) -> int:
            # Built-in groups keep their existing AlloyRAG fields (env +
    ...
    async def _parse_worker(
        ...
        while True:
            ...
            try:
                ...
                # reuse/passthrough (raw/alloyrag/unknown_source) are skipped.
                if (
                ...
                # has already persisted the parsed body to full_docs (alloyrag /
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        body (pending_parse → alloyrag / raw documents start with empty
    ...
    async def process_single_document(
        ...
        *,
        ...
        """
        from alloyrag.parser.routing import parse_process_options
        ...
        async with ctx.semaphore:
            try:
                ...
                # strip the alloyrag marker according to the stored parse_format
                # — parse persisted the body for every engine before enqueue.
                content = strip_alloyrag_doc_prefix(
                ...
                if doc_process_opts.chunking_explicit:
                    ...
                    # keeps ``AlloyRAG.ainsert`` — which intentionally does NOT
                ...
                # parse_format=raw, which silently mislabelled alloyrag docs;
                ...
                if blocks_path and sidecar_backfill_eligible:
                    from alloyrag.sidecar import backfill_chunk_sidecars
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        content_already_extracted = isinstance(content_data, dict) and (
            (
                content_data.get("parse_format") == FULL_DOCS_FORMAT_ALLOYRAG
        ...
        from alloyrag.parser.routing import normalize_parser_engine
        ...
        if intended_engine and stored_engine and intended_engine != stored_engine:
            log_message = (
                ...
                f"ALLOYRAG_PARSER implies engine="
    ...
    async def _persist_parsed_full_docs(
        ...
        # for alloyrag-format) so cross-filename dedup fires regardless of
        # whether the same body was ingested as raw text or via a sidecar.
        # ``strip_alloyrag_doc_prefix`` is a no-op for non-alloyrag formats.
        if fmt in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_ALLOYRAG):
            content_hash = compute_text_content_hash(
                strip_alloyrag_doc_prefix(record.get("content") or "", fmt)
            )
            ...
            if source_name:
                for candidate in matches:
                    if candidate.name == source_name:
                        return str(candidate)
            if parser_engine:
                from alloyrag.parser.routing import filename_parser_directives
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        """Phase 2: Multimodal analysis (VLM). Writes llm_analyze_result to AlloyRAG Document.
        ...
        # Docling, and pre-existing AlloyRAG documents reused from disk
        ...
        if enabled_modalities and tokenizer is not None:
            try:
                from alloyrag.multimodal_context import (
        ...
        try:
            ...
            from alloyrag.llm._vision_utils import (
                image_audit_metadata,
                image_cache_metadata,
                normalize_image_inputs,
                read_image_dimensions,
            )
            from alloyrag.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
            ...
            # in alloyrag.utils).  When the flag is off we must NOT save the
            ...
            def _json_extract(text: str) -> dict[str, Any]:
                ...
                Mirrors :func:`alloyrag.operate._process_json_extraction_result`
            ...
            async def _analyze_text_modality(
                ...
                if tokenizer is not None:
                    from alloyrag.constants import DEFAULT_MAX_EXTRACT_INPUT_TOKENS
                    from alloyrag.multimodal_context import trim_content_to_budget
    ...
    def _build_mm_chunks_from_sidecars(
        ...
        def _render(
            ...
            # ``alloyrag.operate._parse_mm_display_name`` (regex
