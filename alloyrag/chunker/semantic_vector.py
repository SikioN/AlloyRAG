...
The chunker exposed here is ``async`` because AlloyRAG's
...
Caveats:
  - SemanticChunker does NOT enforce a maximum chunk size; the caller's
    ...
    :func:`alloyrag.utils.enforce_chunk_token_limit_before_embedding`.
  - When ``embedding_func`` is ``None`` we log a warning and fall back to
    :func:`alloyrag.chunker.chunking_by_recursive_character` — V's only
...
from alloyrag.constants import DEFAULT_SENTENCE_SPLIT_REGEX
from alloyrag.utils import EmbeddingFunc, Tokenizer, logger
...
class _AsyncEmbeddingFuncAdapter(Embeddings):
    """Bridge a AlloyRAG :class:`EmbeddingFunc` (async) to LangChain's
...
async def chunking_by_semantic_vector(
    ...
    *,
    ...
    Args:
        tokenizer: AlloyRAG tokenizer (used for output token counts).
        ...
        embedding_func: AlloyRAG :class:`EmbeddingFunc`. When ``None``
