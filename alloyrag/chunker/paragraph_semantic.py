"""Paragraph Semantic Chunking for AlloyRAG.

Reads a AlloyRAG ``.blocks.jsonl`` sidecar — produced by any sidecar-emitting
parser (native / mineru / docling): heading-driven blocks, tables kept whole —
and produces a chunk list compatible with
:func:`alloyrag.chunker.chunking_by_token_size`.
...
from alloyrag.table_markup import (
...
from alloyrag.utils import Tokenizer, logger


# ---------------------------------------------------------------------------
# Threshold ratios — derived from the audit-mode constants in
# alloyrag/parser/docx/parse_document.py so the trade-off curves
...
# imported from :mod:`alloyrag.table_markup` so the surrounding-context
...
# Mirrors ``_MD_HEADING_RE`` in ``alloyrag/parser/_markdown.py`` (the same
...
def _tables_sidecar_path(blocks_path: str) -> str | None:
    ...
    Mirrors :func:`alloyrag.utils_pipeline.sidecar_modality_path` naming
...
def _character_split_text(
    ...
    *,
    ...
    """
    from alloyrag.chunker.recursive_character import (
...
def _split_long_block(
    ...
    *,
    ...
    Mirrors :func:`alloyrag.parser.docx.parse_document.split_long_block`,
...
def _is_heading_only(block: dict[str, Any]) -> bool:
    ...
    accepted ambiguity documented in ``alloyrag/parser/_markdown.py`` and is
...
def _merge_small_blocks(
    ...
    *,
    ...
    :func:`alloyrag.parser.docx.parse_document.merge_small_blocks`,
...
def chunking_by_paragraph_semantic(
    ...
    *,
    ...
    Signature follows the AlloyRAG chunker contract — the standard
    ...
    Args:
        tokenizer: AlloyRAG tokenizer (used for all token counting; matches
