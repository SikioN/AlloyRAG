...
from alloyrag.namespace import NameSpace
from alloyrag.chunker import chunking_by_token_size
from alloyrag.operate import (
    extract_entities,
    kg_query,
    naive_query,
    rebuild_knowledge_from_chunks,
)
from alloyrag.utils_pipeline import normalize_document_file_path
from alloyrag.constants import GRAPH_FIELD_SEP
from alloyrag.exceptions import IndexFlushError
from alloyrag.utils import (
    Tokenizer,
    TiktokenTokenizer,
    EmbeddingFunc,
...
from alloyrag.types import KnowledgeGraph
from dotenv import load_dotenv
from alloyrag.pipeline import _PipelineMixin
from alloyrag.kg.factory import get_storage_class
from alloyrag.addon_params import (
    ObservableAddonParams,
    normalize_addon_params,
)
from alloyrag.llm_roles import (
    ROLE_NAMES,
    ROLES,
    RoleLLMConfig,
    RoleSpec,  # noqa: F401  # re-exported via alloyrag/__init__.py
    _optional_env_int,
    _RoleLLMMixin,
    _RoleLLMState,
)
from alloyrag.storage_migrations import _StorageMigrationMixin
...
def _run_sync(
    ...
    * **A different — but still alive — loop than the storages bound to**
      ...
      ``asyncio.Lock`` objects in ``alloyrag.kg.shared_storage`` are bound to
    ...
    *closed* ``owning_loop`` means the loop the storages were initialized on is
    ...
    ``AlloyRAG`` instance; callers that must invoke them from multiple threads
    ...
    Args:
        ...
        owning_loop: The loop the instance's storages were initialized on
            (``AlloyRAG._owning_loop``). ``None`` when storages have not been
    ...
    if (
        ...
        raise RuntimeError(
            f"{sync_name}() must run on the same event loop this AlloyRAG "
            ...
            f"thread. AlloyRAG's shared storage locks are bound to the original "
...
@final
@dataclass
class AlloyRAG(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    ...
    :func:`alloyrag.parser.routing.default_chunker_config`.  Per-doc
    ...
    **When this function is actually invoked.** The chunker dispatch in
    ...
    document's ``process_options``:
      ...
      - If ``process_options`` explicitly contains a chunking selector
        ...
        :mod:`alloyrag.chunker` (``chunking_by_fixed_token`` for ``F``,
    ...
    :attr:`alloyrag.parser.routing.ProcessOptions.chunking_explicit`.

    **Signature** — preserved unchanged from earlier AlloyRAG releases
    ...
    Defaults to :func:`alloyrag.chunker.chunking_by_token_size`.
    """
    ...
    # TODO: Deprecated (AlloyRAG will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, alloyrag will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    def _apply_chunk_size_overlay(self) -> None:
        ...
        2. Strategy-specific env (``CHUNK_F_SIZE`` / ``CHUNK_R_SIZE`` /
           ...
           :func:`alloyrag.parser.routing.default_chunker_config` when it
        ...
        3. Legacy constructor field
           (``AlloyRAG(chunk_token_size=…, chunk_overlap_token_size=…)``).
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        logger.debug(f"AlloyRAG init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple AlloyRAG instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            if default_workspace is None:
                ...
                logger.info(
                    f"Creating AlloyRAG instance with workspace='{self.workspace}' "
                    f"while default workspace is set to '{default_workspace}'"
                )

            # Auto-initialize pipeline_status for this workspace
            from alloyrag.kg.shared_storage import initialize_pipeline_status
    ...
    async def ainsert(
        ...
        The AlloyRAG **server / REST API does not call this method** — it
        ...
        from alloyrag.parser.routing import resolve_chunk_options
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from alloyrag.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from alloyrag.utils_graph import adelete_by_relation
    ...
    async def get_relation_info(
        ...
        """
        from alloyrag.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        """
        from alloyrag.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        """
        from alloyrag.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        """
        from alloyrag.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        """
        from alloyrag.utils_graph import acreate_relation
    ...
    async def aexport_data(
        ...
        """
        from alloyrag.utils import aexport_data as utils_aexport_data
...
# passed through AlloyRAG(addon_params=...). InitVars are not stored as
...
AlloyRAG.addon_params = property(  # type: ignore[attr-defined]
    AlloyRAG._get_addon_params,
    AlloyRAG._set_runtime_addon_params,
)
