...
def create_sample_excel(filepath: str):
    ...
    # Sheet 1: Alloys
    alloys_data = {
        "AlloyName": ["Ni-Superalloy-718", "Ti-6Al-4V-Grade5", "AISI-316L-Steel", "Al-Zn-Mg-Cu-7075"],
        "Composition": [
    ...
    experiments_data = {
        ...
        "AlloyName": ["Ni-Superalloy-718", "Ni-Superalloy-718", "Ti-6Al-4V-Grade5", "AISI-316L-Steel", "Al-Zn-Mg-Cu-7075"],
        "ProcessingMode": [
    ...
    with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
        ...
        pd.DataFrame(experiments_data).to_excel(writer, sheet_name="Experiments", index=False)
...
async def aingest_excel_file(filepath: str):
    ...
    doc_id = f"excel_{os.path.basename(filepath)}"
    logger.info(f"Ingesting {len(custom_kg['entities'])} entities and {len(custom_kg['relationships'])} relations into AlloyRAG...")
...
def ingest_excel_file(filepath: str) -> bool:
    ...
    if loop and loop.is_running():
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            return pool.submit(lambda: asyncio.run(aingest_excel_file(filepath))).result()
    else:
