...
to :data:`alloyrag.constants.DEFAULT_R_SEPARATORS`); only when a single
...
:func:`alloyrag.chunk_schema.strip_internal_multimodal_markup_for_extraction`
...
``alloyrag.operate._process_single_content``) and ensures the
...
from alloyrag.chunk_schema import strip_internal_multimodal_markup_for_extraction
...
from alloyrag.utils import Tokenizer
...
def load_chunk_separators() -> list[str]:
    ...
    :data:`alloyrag.constants.DEFAULT_R_SEPARATORS` on missing / invalid
...
def trim_content_to_budget(
    ...
    *,
    ...
    :data:`alloyrag.constants.DEFAULT_MAX_EXTRACT_INPUT_TOKENS`.  Only ``content``
