...
def _find_legacy_collection(
    ...
    created by older versions of AlloyRAG:

    1. alloyrag_vdb_{namespace} - if model_suffix is provided (HIGHEST PRIORITY)
    2. {workspace}_{namespace} or {namespace} - no matter if model_suffix is provided or not
    3. alloyrag_vdb_{namespace} - fall back value no matter if model_suffix is provided or not (LOWEST PRIORITY)
    ...
    candidates = [
        f"alloyrag_vdb_{namespace}" if model_suffix else None,
        f"{workspace}_{namespace}" if workspace else None,
        f"alloyrag_vdb_{namespace}",
        namespace,
    ]
