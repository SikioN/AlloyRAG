...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    # Get the current logger level from the alloyrag logger
    try:
        from alloyrag.utils import logger

        current_level = logger.getEffectiveLevel()
    except ImportError:
        # Fallback if alloyrag.utils is not available
...
# Later no-arg calls (e.g. AlloyRAG.__post_init__) hit the `_initialized`
