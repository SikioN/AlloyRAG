"""
OpenSearch Storage Implementation for AlloyRAG

This module provides OpenSearch-based storage backends for AlloyRAG,
...
# knobs (alloyrag/kg/mongo_impl.py) so behaviour stays consistent across
...
async def _verify_mirrored_id_mapping(client: AsyncOpenSearch, index_name: str) -> None:
    ...
    cross-shard pagination tiebreaker. Indices created by older AlloyRAG
    ...
    if "__mirrored_id" not in props:
        raise RuntimeError(
            ...
            f"This index was likely created by an older AlloyRAG release. "
...
class OpenSearchKVStorage(BaseKVStorage):
    ...
    async def finalize(self):
        ...
        buffer counts -- otherwise ``AlloyRAG.finalize_storages()`` would
...
class OpenSearchDocStatusStorage(DocStatusStorage):
    ...
    async def _ensure_content_hash_mapping(self) -> None:
        ...
        Indices created by older AlloyRAG releases lack content_hash entirely.
...
class OpenSearchVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        flush, which matches the contract used by other AlloyRAG storage
    ...
    async def get_by_id(self, id: str) -> dict[str, Any] | None:
        ...
        AlloyRAG vector backend (see ``NanoVectorDBStorage.get_by_id``).
