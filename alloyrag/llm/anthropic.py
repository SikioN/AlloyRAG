...
async def anthropic_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    """Call Anthropic Messages API with AlloyRAG-compatible shims.
    ...
    default_headers = {
        "User-Agent": f"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_8) AlloyRAG/{__api_version__}",
    ...
    if image_inputs:
        from alloyrag.llm._vision_utils import normalize_image_inputs
...
# ``alloyrag.llm.voyageai.voyageai_embed``. Keep the old name importable for one
...
def anthropic_embed(*args, **kwargs):
    """Deprecated alias for :func:`alloyrag.llm.voyageai.voyageai_embed`.
    ...
    warnings.warn(
        "alloyrag.llm.anthropic.anthropic_embed is deprecated and will be "
        "removed in a future release. Import "
        "alloyrag.llm.voyageai.voyageai_embed instead.",
        DeprecationWarning,
        stacklevel=2,
    )
