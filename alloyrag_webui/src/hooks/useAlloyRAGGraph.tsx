...
import { queryGraphs } from '@/api/alloyrag'
...
const createSigmaGraph = async (rawGraph: RawGraph | null): Promise<UndirectedGraph | null> => {
  ...
  if (skippedEdges > 0) {
    console.warn(`[useAlloyRAGGraph] ${skippedEdges} edges could not be added to the graph`)
  }
...
const useLightrangeGraph = () => {
  ...
  useEffect(() => {
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      if (lastFetchSignatureRef.current === fetchSignature) {
        console.warn(
          '[useAlloyRAGGraph] Suppressed duplicate graph fetch:',
          fetchSignature,
      ...
      dataPromise
        .then(async (result) => {
          ...
          if (!data || !data.nodes || data.nodes.length === 0) {
            ...
            try {
              ...
              console.error('[useAlloyRAGGraph] createSigmaGraph failed (graph build):', buildError)
            ...
            try {
              ...
              console.error(
                '[useAlloyRAGGraph] a store/sigma subscriber threw while applying the new graph:',
                subscriberError
              )
            ...
            console.log(
              `[useAlloyRAGGraph] sigma graph ready: ${newSigmaGraph?.order} nodes, ${newSigmaGraph?.size} edges`
            )
        ...
        .catch((error) => {
          console.error(
            '[useAlloyRAGGraph] graph load failed (see preceding log for stage):',
            error
          )
          ...
          if (error instanceof GraphBuildError) {
            console.error(
              '[useAlloyRAGGraph] graph build failed (not retrying — deterministic):',
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            retryStateRef.current = { signature: '', attempts: 0 }
          ...
          if (retry.attempts < MAX_FETCH_RETRIES) {
            ...
            console.warn(
              `[useAlloyRAGGraph] graph fetch failed, retry ${retry.attempts}/${MAX_FETCH_RETRIES} in ${delay}ms:`,
              fetchSignature
            )
            if (retryTimerRef.current !== null) {
              clearTimeout(retryTimerRef.current)
            ...
            console.error(
              `[useAlloyRAGGraph] graph fetch failed after ${MAX_FETCH_RETRIES} retries, giving up:`,
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            toast.error(
    ...
    const handleNodeExpand = async () => {
      ...
      try {
        ...
        for (const edgeId of edgesToAdd) {
          ...
          if (dynamicId === null) {
            console.warn('[useAlloyRAGGraph] could not add expansion edge:', newEdge.id)
            continue
          }
  ...
  const alloyrageGraph = useCallback(() => {
  ...
  return { alloyrageGraph, getNode, getEdge }
}
