...
import { checkHealth, AlloyRAGStatus } from '@/api/alloyrag'
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  const token = localStorage.getItem('ALLOYRAG-API-TOKEN');
  const coreVersion = localStorage.getItem('ALLOYRAG-CORE-VERSION');
  const apiVersion = localStorage.getItem('ALLOYRAG-API-VERSION');
  const webuiTitle = localStorage.getItem('ALLOYRAG-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('ALLOYRAG-WEBUI-DESCRIPTION');
  const lastTokenRenewal = localStorage.getItem('ALLOYRAG-LAST-TOKEN-RENEWAL');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('ALLOYRAG-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('ALLOYRAG-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('ALLOYRAG-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('ALLOYRAG-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('ALLOYRAG-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('ALLOYRAG-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('ALLOYRAG-WEBUI-DESCRIPTION');
      }
      ...
      localStorage.setItem('ALLOYRAG-LAST-TOKEN-RENEWAL', formattedTime);
    ...
    logout: () => {
      localStorage.removeItem('ALLOYRAG-API-TOKEN');
      localStorage.removeItem('ALLOYRAG-LAST-TOKEN-RENEWAL');
