...
// Types
export type AlloyRAGNodeType = {
  id: string
  labels: string[]
  properties: Record<string, any>
}

export type AlloyRAGEdgeType = {
  id: string
  source: string
  target: string
  type: string
...
export type AlloyRAGGraphType = {
  nodes: AlloyRAGNodeType[]
  edges: AlloyRAGEdgeType[]
}

export type AlloyRAGQueueStatus = {
  available: boolean
  queue_name?: string
  max_async?: number
...
export type AlloyRAGRoleLLMConfig = {
  binding?: string | null
  model?: string | null
  host?: string | null
  max_async?: number
...
export type AlloyRAGStatus = {
  ...
  configuration: {
    ...
    role_llm_config?: Record<string, AlloyRAGRoleLLMConfig>
  ...
  llm_queue_status?: Record<string, AlloyRAGQueueStatus>
  embedding_queue_status?: AlloyRAGQueueStatus
  rerank_queue_status?: AlloyRAGQueueStatus
  keyed_locks?: {
    process_id: number
    cleanup_performed: {
      mp_cleaned: number
...
export type AlloyRAGDocumentsScanProgress = {
  is_scanning: boolean
  current_file: string
  ...
  refreshTokenPromise = (async () => {
    try {
      ...
      if (response.data.access_token && !response.data.auth_configured) {
        ...
        // Update localStorage
        localStorage.setItem('ALLOYRAG-API-TOKEN', newToken);
        // Update auth state
  ...
  async (error: AxiosError) => {
    if (error.response) {
      if (error.response?.status === 401) {
        ...
        const authStore = useAuthStore.getState();
        const currentToken = localStorage.getItem('ALLOYRAG-API-TOKEN');
...
export const checkHealth = async (): Promise<
  AlloyRAGStatus | { status: 'error'; message: string }
> => {
  try {
...
export const getDocumentsScanProgress = async (): Promise<AlloyRAGDocumentsScanProgress> => {
