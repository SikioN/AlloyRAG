...
let apiModule: typeof import('./alloyrag')

beforeAll(async () => {
  apiModule = await import('./alloyrag')
})
...
describe('queryTextStream — auth headers', () => {
  test('includes Bearer token when stored', async () => {
    storageData.set('ALLOYRAG-API-TOKEN', 'test-jwt-token')
...
describe('queryTextStream — guest-token 401 retry', () => {
  test('retries with refreshed guest token on 401', async () => {
    storageData.set('ALLOYRAG-API-TOKEN', 'expired-guest-token')
