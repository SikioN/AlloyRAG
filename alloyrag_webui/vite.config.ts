...
/**
 * Inject `<script>window.__ALLOYRAG_CONFIG__ = ...</script>` into index.html.
 *
 * This mirrors what the FastAPI server does at request time in production
 * (see `SmartStaticFiles._inject_runtime_config` in
 * `alloyrag/api/alloyrag_server.py`). Doing it in dev too means the SPA
 ...
 */
function alloyragRuntimeConfigPlugin(env: Record<string, string>): Plugin {
  ...
  const snippet = `<script>window.__ALLOYRAG_CONFIG__ = ${payload};</script>`

  return {
    name: 'alloyrag-dev-runtime-config',
    apply: 'serve',
    transformIndexHtml(html: string) {
      return html.replace('<!-- __ALLOYRAG_RUNTIME_CONFIG__ -->', snippet)
    }
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  // running with ALLOYRAG_API_PREFIX=/site01.
  const devApiPrefix = normalizeApiPrefix(env.VITE_DEV_API_PREFIX)

  return {
    plugins: [react(), tailwindcss(), alloyragRuntimeConfigPlugin(env)],
    resolve: {
      alias: {
    ...
    base: './',
    build: {
      outDir: path.resolve(__dirname, '../alloyrag/api/webui'),
      emptyOutDir: true,
