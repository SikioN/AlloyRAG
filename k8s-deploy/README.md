...
This is the Helm chart for AlloyRAG, used to deploy AlloyRAG services on a Kubernetes cluster.

There are two recommended deployment methods for AlloyRAG:
...
* **Helm** (v3.x+)
  * Kubernetes package manager used to install AlloyRAG.
...
You can deploy AlloyRAG using either the provided convenience script or direct Helm commands. Both methods configure the same environment variables defined in the `alloyrag/values.yaml` file.
...
helm upgrade --install alloyrag ./alloyrag \
  --namespace rag \
  --set-string env.ALLOYRAG_KV_STORAGE=JsonKVStorage \
  --set-string env.ALLOYRAG_VECTOR_STORAGE=NanoVectorDBStorage \
  --set-string env.ALLOYRAG_GRAPH_STORAGE=NetworkXStorage \
  --set-string env.ALLOYRAG_DOC_STATUS_STORAGE=JsonDocStatusStorage \
...
kubectl --namespace rag port-forward svc/alloyrag-dev 9621:9621
...
AlloyRAG and its databases are deployed within the same Kubernetes cluster, making configuration straightforward.
...
```bash
# 1. Run this port-forward command in your terminal:
kubectl --namespace rag port-forward svc/alloyrag 9621:9621
...
You can configure AlloyRAG's resource usage by modifying the `values.yaml` file:
...
The `env` section in the `values.yaml` file contains all environment configurations for AlloyRAG, similar to a `.env` file. When using helm upgrade or helm install commands, you can override these with the --set flag.
...
env:
  ...
  ALLOYRAG_KV_STORAGE: PGKVStorage              # Key-value storage type
  ALLOYRAG_VECTOR_STORAGE: PGVectorStorage      # Vector storage type
  ALLOYRAG_GRAPH_STORAGE: Neo4JStorage          # Graph storage type
  ALLOYRAG_DOC_STATUS_STORAGE: PGDocStatusStorage  # Document status storage type
```
...
- For more customized configurations, please refer to the official AlloyRAG documentation
