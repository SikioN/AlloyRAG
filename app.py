...
from alloyrag.url_crawler import crawl_and_index_url, acrawl_and_index_url
from alloyrag.ingest_manager import scan_and_ingest_inputs, ascan_and_ingest_inputs, STATUS_FILE, get_ingested_states
...
async def query_rag(request: QueryRequest):
    ...
    try:
        ...
        if answer is None:
            logger.error(f"RAG query returned None - possible LLM timeout or context overflow")
...
async def crawl_url(request: CrawlRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if success:
            return {"status": "success", "message": f"URL {request.url} crawled and indexed successfully."}
        else:
            raise HTTPException(status_code=400, detail="Failed to crawl URL. Check logs for details.")
    except Exception as e:
...
async def get_system_status():
    ...
    return {
        "is_indexing_busy": is_indexing_busy,
        "files": files
    }
    ...
    try:
        ...
        for node_id in nx_graph.nodes:
            ...
            nodes.append({
                ...
                "value": nx_graph.degree(node_id) # Node size based on degree (importance)
            })
...
async def upload_document_custom(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
...
async def get_pipeline_status():
    return {
        "busy": is_indexing_busy,
        "active": is_indexing_busy,
        "pending_enqueues": 0
    }
...
app.include_router(create_document_routes(rag_instance, doc_manager_instance, api_key_val))
...
async def serve_webui_index():
    index_path = os.path.join(os.path.dirname(__file__), "alloyrag", "api", "webui", "index.html")
    ...
    runtime_config_script = f"<script>window.__ALLOYRAG_CONFIG__ = {_runtime_config_payload};</script>"
    
    content = content.replace("<!-- __ALLOYRAG_RUNTIME_CONFIG__ -->", runtime_config_script)
