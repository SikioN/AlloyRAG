...
Use the interactive setup wizard when you want AlloyRAG to guide you through the configuration instead of editing `.env` by hand.
...
The setup wizard helps you configure AlloyRAG in three parts:
...
- If you choose any wizard-managed Docker service, the wizard also prepares AlloyRAG for the Docker startup path.
...
Use this when you want AlloyRAG running with the least amount of setup and you already have remote model endpoints or API keys.
...
Use this when you want AlloyRAG to run local inference services for embedding and/or reranking through Docker.
...
This starts the generated Docker-based AlloyRAG stack together with the selected local services.
...
- If you pointed AlloyRAG at external databases, make sure those services are reachable before starting AlloyRAG.
...
- If the stack uses Docker, recreate the AlloyRAG service with your compose file
- If the stack runs on the host, restart `alloyrag-server`
...
Use this before exposing AlloyRAG beyond localhost. It reports risky setups such as missing authentication, weak or missing JWT secrets, unsafe whitelist settings, or unresolved sensitive placeholders.
