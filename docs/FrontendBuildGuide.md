...
The AlloyRAG project includes a React-based WebUI frontend. This guide explains how frontend building works in different scenarios.
...
- Frontend build output goes to `alloyrag/api/webui/`
...
# Output: dist/alloyrag_hku-*.whl and dist/alloyrag_hku-*.tar.gz
```
...
|----------|---------|-------------------|-----------------|
| From PyPI | `pip install alloyrag-hku[api]` | Included | No (already installed) |
...
```
AlloyRAG/
├── alloyrag_webui/          # Frontend source code
│   ├── src/                 # React components
│   ├── package.json         # Dependencies
│   └── vite.config.ts       # Build configuration
│       └── outDir: ../alloyrag/api/webui  # Build output
│
├── alloyrag/
│   └── api/
...
└── .gitignore               # Excludes alloyrag/api/webui/* (except .gitkeep)
```
...
```bash
cd alloyrag_webui && bun run build
```
...
- Or reinstall: `pip uninstall alloyrag-hku && pip install ".[api]"`

### Q: Where are the built frontend files?

**A:** In `alloyrag/api/webui/` after running `bun run build`
