...
This tool is used to locally debug any parsing engine in AlloyRAG's registry (the built-in `native` / `legacy` / `mineru` / `docling`, plus third-party engines registered via the `alloyrag.parsers` entry point — see `docs/ThirdPartyParser-zh.md`). It drives the same registry dispatch path as the pipeline worker (`get_parser(engine).parse(...)`) for a **single file** and outputs the parsing artifacts (sidecar and raw cache) into a **flat directory layout**. Compared with the production ingestion directory, the only differences are:
...
```bash
python -m alloyrag.parser.cli <input_file> \
    --engine <engine> \
...
```bash
python -m alloyrag.parser.cli ./inputs/workspace/sample.docx --engine native
...
python -m alloyrag.parser.cli ./inputs/workspace/sample.pdf --engine mineru
...
```bash
python -m alloyrag.parser.cli ./inputs/workspace/sample.pdf \
...
This CLI drives the same registry dispatch path as the pipeline parse worker — `get_parser(engine).parse(ParseContext(rag, ...))` (with `rag` being the lightweight stand-in in `alloyrag/parser/debug.py`), so:
