...
AlloyRAG's parsing layer dispatches all parsing engines through a unified `BaseParser` contract plus the central engine registry (`alloyrag/parser/registry.py`). Built-in engines (`native` / `legacy` / `mineru` / `docling`) and third-party engines use the exact same dispatch path: both pipeline workers and the debug CLI drive parsing through `get_parser(engine).parse(ParseContext(...))`, with **no special cases for built-in engines**. Therefore, a third-party package only needs to do two things:

1. **Implement** a `BaseParser` subclass;
2. **Register** a `ParserSpec` (automatic discovery through the `alloyrag.parsers` entry point is recommended).

Once that is done, the engine automatically gets: a dedicated (or shared) parsing concurrency pool, three engine-selection methods (filename hints / `ALLOYRAG_PARSER` routing rules / API `parse_engine` parameter), suffix capability validation, and single-file debug support through `python -m alloyrag.parser.cli --engine <name>`.

> For architecture background, see RFC #3197; for the sidecar file format, see `docs/AlloyRAGSidecarFormat.md`; for CLI usage, see `docs/ParserDebugCLI.md`.
...
### 2.1 Contract (`alloyrag/parser/base.py`)
...
| Member | Description |
|---|---|
| `ctx.rag` | AlloyRAG instance (used for `_persist_parsed_full_docs`, etc.) |
...
`ParseResult` fields: `doc_id` / `file_path` / `parse_format` (`"raw"` or `"alloyrag"`) / `content` / `blocks_path` (`""` when there is no sidecar) / `parse_engine` / `parse_stage_skipped` (skipped scenarios such as cache hits) / `parse_warnings` (non-fatal warnings, persisted to `doc_status.metadata`).
...
See `alloyrag/parser/legacy/parser.py` (`LegacyParser`). Core skeleton:
...
**B. Local parser that produces a sidecar - inherit `NativeParserBase`** (`alloyrag/parser/native_base.py`)
...
Optional overrides: `validate_source` (by default only requires the file to exist), `surface_warnings` (maps extraction warnings to `parse_warnings`). Reference implementation: `alloyrag/parser/docx/parser.py`.

**C. External parsing service (download raw bundle + cache) - inherit `ExternalParserBase`** (`alloyrag/parser/external/_base.py`)
...
class MyExternalParser(ExternalParserBase):
    ...
    force_reparse_env = "ALLOYRAG_FORCE_REPARSE_MYENGINE"
...
Optional override: `validate_ir` (post-build validation, for example failing on zero blocks). Reference implementations: `alloyrag/parser/external/mineru/parser.py`, `.../docling/parser.py`.
...
```python
from alloyrag.parser.registry import ParserSpec, register_parser
...
- Worker count for a group: built-in groups (`native` / `mineru` / `docling`) are determined by AlloyRAG instance fields `max_parallel_parse_*` (constructor overrides are supported); a third-party dedicated group uses the `concurrency` value from the group's sole owner spec; **only one** spec in a group may declare `concurrency`, otherwise batch startup fails;
...
AlloyRAG automatically discovers third-party engines through the `alloyrag.parsers` entry-point group (`alloyrag/parser/plugins.py`). A third-party package only needs to:
...
```python
# my_pkg/alloyrag_plugin.py - keep imports cheap: do not import parser implementations here
import os
from alloyrag.parser.registry import ParserSpec, register_parser
...
After `pip install my-pkg`, it works immediately without changing AlloyRAG code:

- **API Server**: `create_app()` calls `load_third_party_parsers()` **before** validating `ALLOYRAG_PARSER` routing rules, so routing rules can directly reference third-party engine names (for example, `ALLOYRAG_PARSER="foo:myengine"`). File-type guarding for uploads and scans is **derived entirely from the registry plus routing at runtime**. The criterion is "can this file route to an engine that supports it": a bare suffix (no hint) requires a `ALLOYRAG_PARSER` rule that routes it to your engine (otherwise the default `legacy` engine cannot handle it, so the file is rejected instead of being accepted and later FAILED during parsing); uploads with a filename hint (for example, `report.[myengine].foo`) pass without a rule; unique suffixes of engines whose endpoints are not configured do not participate (the same rule used by built-in mineru/docling image formats). **Practical recommendation: when publishing a third-party engine, tell users to configure the matching `ALLOYRAG_PARSER="foo:myengine"` rule in deployment docs**, so bare-filename uploads and directory scans work automatically;
- **Debug CLI**: `python -m alloyrag.parser.cli sample.foo --engine myengine` works directly (`main()` loads plugins before building `--engine` choices). For engines without sidecars (`blocks_path=""`), the CLI prints a plain-text summary instead of a block summary; engines inheriting `ExternalParserBase` automatically get raw cache display and `--force-reparse` support;
- **Embedded library usage** (using the `AlloyRAG` class directly without going through the server or CLI): call this once before building the pipeline:
...
> If you do not want to publish a package, you can skip entry points and directly call `register_parser(...)` in your own startup script before starting/calling AlloyRAG. The registry is an in-process module singleton, so the effect is the same, just without "install and it works" behavior.

## 5. Routing: Making Documents Use Your Engine

Engine selection priority (`alloyrag/parser/routing.py`):

1. **Filename hint**: `report.[myengine].foo` (processing options are allowed, such as `report.[myengine-iet].foo`);
2. **`ALLOYRAG_PARSER` rules**: for example, `ALLOYRAG_PARSER="foo:myengine,pdf:mineru"` (matched by suffix glob; the first match wins);
3. **Default**: `legacy`.
...
- **Entry-point loading logic**: see `tests/parser/test_plugins.py` (monkeypatch `alloyrag.parser.plugins.entry_points` to inject a fake entry point; the `plugins._loaded` flag must be reset).
