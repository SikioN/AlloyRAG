...
AlloyRAG can be configured using environment variables in the `.env` file:
...
The interactive setup keeps `.env` host-usable. Container-only hostnames such as `postgres` or `host.docker.internal`, along with staged SSL paths under `/app/data/certs/`, are injected into the generated `docker-compose.final.yml` for the `alloyrag` service instead of being persisted back into `.env`.
...
AlloyRAG Server uses the following paths for data storage:
...
The official images run the server as a non-root user (`alloyrag`, uid/gid `1000`) to satisfy CIS Docker Benchmark 4.1. The behavior is designed so existing deployments keep working on upgrade:
...
If AlloyRAG runs in Docker while vLLM runs on the host, the generated compose file rewrites those endpoints to:
...
The image no longer ships fixed credentials; on first start it creates the user, password, and database from the `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB` environment variables. The setup wizard prompts for these values (defaulting to `rag` / `rag` / `alloyrag`) and injects them into the generated `docker-compose.final.yml`, so you can choose any user, password, and database name.
...
Official AlloyRAG images published to GitHub Container Registry by GitHub Actions are signed with Sigstore Cosign using GitHub OIDC keyless signing.
