# Programming With AlloyRAG Core

> If you want to integrate AlloyRAG into your project, we recommend using the REST API provided by the AlloyRAG Server. AlloyRAG Core is intended for embedded applications or researchers conducting studies and evaluations.
...
import os
import asyncio
from alloyrag import AlloyRAG, QueryParam
from alloyrag.llm.openai import gpt_4o_mini_complete, gpt_4o_complete, openai_embed
from alloyrag.utils import setup_logger

setup_logger("alloyrag", level="INFO")
...
**AlloyRAG requires explicit initialization before use.** You must call `await rag.initialize_storages()` after creating a AlloyRAG instance, otherwise you will encounter errors.


## AlloyRAG Init Parameters

**Parameters**

| **Parameter** | **Type** | **Explanation** | **Default** |
| -------------- | ---------- | ----------------- | ------------- |
| **working_dir** | `str` | Directory where the cache will be stored | `alloyrag_cache+timestamp` |
| **workspace** | str | Workspace name for data isolation between different AlloyRAG Instances | |
...
`addon_params` is a live configuration mapping on each `AlloyRAG` instance. AlloyRAG currently reads the fields below; unknown custom keys may remain in the dict, but core AlloyRAG behavior does not use them.
...
When you create a `AlloyRAG` object, `addon_params` is normalized before storage initialization:

- If `addon_params` is omitted, AlloyRAG builds defaults from `SUMMARY_LANGUAGE`, `ENTITY_TYPE_PROMPT_FILE`, and the chunker-related `CHUNK_*` environment variables.
...
`rag.addon_params` is an observable mapping. Top-level updates mark the derived prompt cache dirty; the cache is refreshed the next time AlloyRAG builds runtime config for extraction or query work.
...
```python
class QueryParam:
    """Configuration parameters for query execution in AlloyRAG."""
...
AlloyRAG requires LLM and Embedding models for document indexing and querying. During initialization, inject the relevant model functions into AlloyRAG.
...
AlloyRAG supports OpenAI-like chat/embeddings APIs:

```python
import os
import numpy as np
from alloyrag.utils import wrap_embedding_func_with_attrs
from alloyrag.llm.openai import openai_complete_if_cache, openai_embed
...
# Initialize AlloyRAG with Hugging Face model
rag = AlloyRAG(
    working_dir=WORKING_DIR,
...
import numpy as np
from alloyrag.utils import wrap_embedding_func_with_attrs
from alloyrag.llm.ollama import ollama_model_complete, ollama_embed
...
# Initialize AlloyRAG with Ollama model
rag = AlloyRAG(
    working_dir=WORKING_DIR,
...
AlloyRAG requires at least 32k context tokens. Ollama defaults to 8k. Two approaches:
...
AlloyRAG supports integration with LlamaIndex (`llm/llama_index_impl.py`):

```python
import asyncio
from alloyrag import AlloyRAG
from alloyrag.llm.llama_index_impl import llama_index_complete_if_cache, llama_index_embed
...
- [Direct OpenAI Example](examples/unofficial-sample/alloyrag_llamaindex_direct_demo.py)
- [LiteLLM Proxy Example](examples/unofficial-sample/alloyrag_llamaindex_litellm_demo.py)
- [LiteLLM Proxy with Opik Example](examples/unofficial-sample/alloyrag_llamaindex_litellm_opik_demo.py)
...
import numpy as np
from alloyrag.utils import wrap_embedding_func_with_attrs
from alloyrag.llm.azure_openai import azure_openai_complete_if_cache, azure_openai_embed
...
import numpy as np
from alloyrag.utils import wrap_embedding_func_with_attrs
from alloyrag.llm.gemini import gemini_model_complete, gemini_embed
...
Inject one of these functions into the `rerank_model_func` attribute of the AlloyRAG object. For detailed usage, refer to `examples/rerank_example.py`.

### User Prompt vs. Query

When using AlloyRAG for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. The `user_prompt` parameter in `QueryParam` does not participate in the RAG retrieval phase — it guides the LLM on how to process the retrieved results after the query is completed.
...
Example connection configurations for each storage type can be found in the repository's `env.example` file. The database instance in the connection string must be created beforehand — AlloyRAG only creates tables within the instance, not the instance itself.
...
- How to start: see [examples/alloyrag_gemini_postgres_demo.py](https://github.com/SikioN/AlloyRAG/blob/main/examples/alloyrag_gemini_postgres_demo.py)
...
MongoDB provides a one-stop storage solution for AlloyRAG with native KV storage and vector storage. AlloyRAG uses MongoDB collections to implement a simple graph storage.
...
AlloyRAG supports Redis as KV storage. Configure persistence and memory usage carefully. Recommended Redis configuration:
...
OpenSearch provides a unified storage solution for all four AlloyRAG storage types (KV, Vector, Graph, DocStatus). It offers native k-NN vector search, full-text search, and horizontal scalability without cloud-only restrictions.
...
python examples/alloyrag_openai_opensearch_graph_demo.py
```

6. Visualize the knowledge graph via AlloyRAG WebUI:
```bash
ALLOYRAG_KV_STORAGE=OpenSearchKVStorage \
ALLOYRAG_DOC_STATUS_STORAGE=OpenSearchDocStatusStorage \
ALLOYRAG_GRAPH_STORAGE=OpenSearchGraphStorage \
ALLOYRAG_VECTOR_STORAGE=OpenSearchVectorDBStorage \
LLM_BINDING=openai \
...
The `workspace` parameter ensures data isolation between different AlloyRAG instances. Once initialized, the `workspace` is immutable.
...
For a practical demonstration of managing multiple isolated knowledge bases, see [Workspace Demo](examples/alloyrag_gemini_workspace_demo.py).
...
AlloyRAG supports comprehensive knowledge graph management: create, edit, and delete entities and relationships.
...
AlloyRAG provides comprehensive deletion capabilities.
...
1. **`AttributeError: __aenter__`**
   ...
   - **Solution**: Call `await rag.initialize_storages()` after creating the AlloyRAG instance
