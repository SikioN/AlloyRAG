# AlloyRAG Offline Deployment Guide

This guide provides comprehensive instructions for deploying AlloyRAG in offline environments where internet access is limited or unavailable.

If you deploy AlloyRAG using Docker, there is no need to refer to this document, as the AlloyRAG Docker image is pre-configured for offline operation.

> Software packages requiring `transformers`, `torch`, or `cuda` will not be included in the offline dependency group. Consequently, document extraction tools such as Docling, as well as local LLM models like Hugging Face and LMDeploy, are outside the scope of offline installation support. These high-compute-resource-demanding services should not be integrated into AlloyRAG. Docling will be decoupled and deployed as a standalone service.
...
AlloyRAG uses dynamic package installation (`pipmaster`) for optional features based on file types and configurations. In offline environments, these dynamic installations will fail. This guide shows you how to pre-install all necessary dependencies and cache files.

### What Gets Dynamically Installed?

AlloyRAG dynamically installs packages for:
...
pip install alloyrag-hku[offline]

# Download tiktoken cache
alloyrag-download-cache

# Create offline package
pip download alloyrag-hku[offline] -d ./offline-packages
tar -czf alloyrag-offline.tar.gz ./offline-packages ~/.tiktoken_cache

# Transfer to offline server
scp alloyrag-offline.tar.gz user@offline-server:/path/to/

# Offline environment: Install
tar -xzf alloyrag-offline.tar.gz
pip install --no-index --find-links=./offline-packages alloyrag-hku[offline]
...
AlloyRAG provides flexible dependency groups for different use cases:
...
pip install alloyrag-hku[api,offline-storage]
...
After installing AlloyRAG, use the built-in command:
...
alloyrag-download-cache --cache-dir ./tiktoken_cache
...
```bash
# 1. Install AlloyRAG with offline dependencies
pip install alloyrag-hku[offline]

# 2. Download tiktoken cache
alloyrag-download-cache --cache-dir ./offline_cache/tiktoken

# 3. Download all Python packages
pip download alloyrag-hku[offline] -d ./offline_cache/packages

# 4. Create archive for transfer
tar -czf alloyrag-offline-complete.tar.gz ./offline_cache

# 5. Verify contents
tar -tzf alloyrag-offline-complete.tar.gz | head -20
```
...
```bash
# Using scp
scp alloyrag-offline-complete.tar.gz user@offline-server:/tmp/

# Or using USB/physical media
# Copy alloyrag-offline-complete.tar.gz to USB drive
```
...
# 1. Extract archive
cd /tmp
tar -xzf alloyrag-offline-complete.tar.gz

# 2. Install Python packages
pip install --no-index \
    --find-links=/tmp/offline_cache/packages \
    alloyrag-hku[offline]
...
```bash
# Test Python import
python -c "from alloyrag import AlloyRAG; print('✓ AlloyRAG imported')"

# Test tiktoken
python -c "from alloyrag.utils import TiktokenTokenizer; t = TiktokenTokenizer(); print('✓ Tiktoken working')"
...
pip install alloyrag-hku[offline-storage]

# For LLM providers:
pip install alloyrag-hku[offline-llm]
```
...
pip install alloyrag-hku[offline]  # Install all offline deps
```
