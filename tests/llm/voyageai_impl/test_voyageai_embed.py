"""Unit tests for alloyrag.llm.voyageai.
...
def patched_async_client(fake_voyage_response):
    ...
    with patch(
        "alloyrag.llm.voyageai.voyageai.AsyncClient", return_value=fake_client
    ) as m:
        yield captured, m
...
async def test_voyageai_embed_maps_context_to_input_type(patched_async_client):
    """AlloyRAG query/document context should drive VoyageAI's input_type."""
    captured, _ = patched_async_client
    from alloyrag.llm.voyageai import voyageai_embed
...
def test_anthropic_embed_deprecation_shim():
    ...
    from alloyrag.llm.anthropic import anthropic_embed  # must not ImportError
