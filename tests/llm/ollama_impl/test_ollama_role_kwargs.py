...
class _FakeRag:
    def __init__(self):
        ...
        self.ollama_server_infos = SimpleNamespace(
            ALLOYRAG_MODEL="alloyrag:latest",
            ALLOYRAG_CREATED_AT="2026-03-14T00:00:00Z",
            ALLOYRAG_SIZE=0,
        )
...
def _make_client(monkeypatch) -> tuple[TestClient, _FakeRag]:
    ...
    for module_name in [
        "alloyrag.api.config",
        "alloyrag.api.auth",
        "alloyrag.api.utils_api",
        "alloyrag.api.routers",
        "alloyrag.api.routers.ollama_api",
    ]:
        sys.modules.pop(module_name, None)
    ollama_api_module = importlib.import_module("alloyrag.api.routers.ollama_api")
