...
from alloyrag.utils import repair_vlm_json_escape_damage


@pytest.fixture
def _propagate_alloyrag_logger(monkeypatch):
    """``alloyrag.utils.logger`` sets ``propagate = False``; restore
...
def test_whitespace_class_damage_is_logged_not_rewritten(
    caplog, _propagate_alloyrag_logger
):
    ...
    with caplog.at_level(logging.WARNING, logger="alloyrag"):
...
@pytest.mark.offline
def test_legitimate_whitespace_is_not_flagged(caplog, _propagate_alloyrag_logger):
...
def test_prompts_require_double_escaped_backslashes():
    ...
    from alloyrag.prompt_multimodal import MULTIMODAL_PROMPTS
...
async def test_extraction_json_result_repairs_latex_escape_damage():
    ...
    from alloyrag.operate import _process_json_extraction_result

    raw_response = (
        '{"entities": [{"name": "AlloyRAG", "type": "Other", '
        '"description": "成本为 $\\frac{610}{C}$ 次调用"}], '
        '"relationships": [{"source": "AlloyRAG", "target": "GraphRAG", '
        '"keywords": "cost", '
    ...
    (entity_list,) = [nodes[k] for k in nodes if k == "AlloyRAG"]
