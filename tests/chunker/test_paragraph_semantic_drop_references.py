...
from alloyrag.constants import DEFAULT_P_REFERENCES_HEADINGS
from alloyrag.utils import Tokenizer, TokenizerInterface, logger as _alloyrag_logger
...
def test_drop_references_emits_info_log(tmp_path, caplog):
    ...
    # The alloyrag logger sets propagate=False, so caplog can't see it by
    # default — enable propagation for the duration of the call.
    original_propagate = _alloyrag_logger.propagate
    _alloyrag_logger.propagate = True
    try:
        with caplog.at_level(logging.INFO, logger=_alloyrag_logger.name):
            chunking_by_paragraph_semantic(
                tokenizer,
                "",
                2000,
        ...
        _alloyrag_logger.propagate = original_propagate
