...
from alloyrag.chunker.paragraph_semantic import (
...
from alloyrag.utils import Tokenizer, TokenizerInterface
...
def _propagate_alloyrag_logger(monkeypatch):
    # The ``alloyrag`` logger sets ``propagate=False`` (see alloyrag.utils), so
...
def test_extract_table_id_variants():
    from alloyrag.table_markup import extract_table_id
...
def test_inject_header_into_table_slice_json_and_html():
    from alloyrag.chunker.paragraph_semantic import _inject_header_into_table_slice
...
def test_split_table_text_multi_row_header_pinned_not_duplicated():
    ...
    from alloyrag.table_markup import TABLE_TAG_RE
