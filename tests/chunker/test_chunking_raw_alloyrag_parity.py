"""F-chunking parity between raw and alloyrag formats.

After the F-chunking unification, ``apipeline_process_enqueue_documents``
strips the ``{{LRdoc}}`` marker from alloyrag-format content and feeds the
...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as an already-parsed alloyrag full_docs row pulled
  back through the parse queue on resume (ReuseParser) — the only
  production path for alloyrag rows now that the ``docs_format="alloyrag"``
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is ``"alloyrag"`` for natively-parsed docs —
...
* T6: a ``pending_parse`` document that resolves to alloyrag at parse
...
* T7: a raw document whose body *literally* starts with ``{{LRdoc}}``
  ...
  unconditional ``strip_alloyrag_doc_prefix`` at the chunking boundary
...
from alloyrag import AlloyRAG, ROLES, RoleLLMConfig
from alloyrag.constants import (
    FULL_DOCS_FORMAT_ALLOYRAG,
    FULL_DOCS_FORMAT_PENDING_PARSE,
    ALLOYRAG_DOC_CONTENT_PREFIX,
)
...
from alloyrag.utils_pipeline import make_alloyrag_doc_content
...
def _new_rag(tmp_path: Path, **kwargs) -> AlloyRAG:
...
def _attach_chunking_spy(rag: AlloyRAG) -> dict:
...
# ---------------------------------------------------------------------------
# T1 — parity: raw vs alloyrag produce identical chunking input
# ---------------------------------------------------------------------------


async def _seed_alloyrag_row(rag: AlloyRAG, doc_id: str, *, body: str, file_path: str):
    """Seed an already-parsed alloyrag full_docs row + a PENDING doc_status.

    Mirrors how alloyrag-format rows exist in production: written by the
    ...
    await rag.full_docs.upsert(
        {
            doc_id: {
                "content": make_alloyrag_doc_content(body),
                "file_path": file_path,
                "parse_format": FULL_DOCS_FORMAT_ALLOYRAG,
            }
        }
    )
...
@pytest.mark.offline
def test_chunking_input_parity_raw_vs_alloyrag(tmp_path):
    """Same body text must reach ``chunking_func`` with byte-identical input
    whether it arrives as raw or as a resumed alloyrag full_docs row."""
    ...
    async def _run():
        ...
        assert spy_lr["calls"] >= 1, "alloyrag doc never reached chunking_func"
        assert spy_lr["input"] == spy_raw["input"] == expected_merged, (
            "chunking_func received different inputs for raw vs alloyrag; "
            f"raw={spy_raw['input']!r}\nlr={spy_lr['input']!r}"
        )
        assert not spy_lr["input"].startswith(ALLOYRAG_DOC_CONTENT_PREFIX), (
...
def _stub_docx_blocks(monkeypatch, body_paragraphs: list[str]) -> None:
    ...
    persists the alloyrag-format full_docs row."""
    ...
    monkeypatch.setattr(
        "alloyrag.parser.docx.parse_document.extract_docx_blocks",
        _stub_extract,
    )
...
def test_full_docs_content_carries_full_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert full_doc["content"] == ALLOYRAG_DOC_CONTENT_PREFIX + body
            assert full_doc.get("parse_format") == FULL_DOCS_FORMAT_ALLOYRAG
            assert full_doc.get("sidecar_location"), (
                "native parse must record the sidecar_location on the "
                "alloyrag full_docs row"
            )
            ...
            assert not summary.startswith(ALLOYRAG_DOC_CONTENT_PREFIX)
        finally:
...
def test_extraction_meta_records_alloyrag_parse_format(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert metadata.get("parse_format") == FULL_DOCS_FORMAT_ALLOYRAG, (
                f"doc_status.metadata.parse_format="
                f"{metadata.get('parse_format')!r}; "
                f"expected {FULL_DOCS_FORMAT_ALLOYRAG!r} so the multimodal "
...
class _ListHandler(logging.Handler):
    ...
    The ``alloyrag`` logger has ``propagate = False`` so pytest's caplog
...
def test_explicit_R_dispatches_to_recursive_character(tmp_path, monkeypatch):
    ...
    import alloyrag.chunker as chunker_pkg
    from alloyrag.chunker import chunking_by_recursive_character as real_r
    ...
    # The dispatcher does ``from alloyrag.chunker import …`` inside the
    ...
    async def _run():
        ...
        alloyrag_logger = logging.getLogger("alloyrag")
        list_handler = _ListHandler()
        list_handler.setLevel(logging.WARNING)
        alloyrag_logger.addHandler(list_handler)
        try:
            await rag.apipeline_enqueue_documents(
            ...
            alloyrag_logger.removeHandler(list_handler)
            await rag.finalize_storages()
...
def test_explicit_V_dispatches_to_semantic_vector(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        assert captured.get("embedding_func") is rag.embedding_func, (
            "dispatcher must hand the AlloyRAG embedding_func to the V chunker"
        )
        assert legacy_spy["calls"] == 0, (
...
# ---------------------------------------------------------------------------
# T6 — pending_parse → alloyrag summary is populated after PROCESSED
# ---------------------------------------------------------------------------


@pytest.mark.offline
def test_pending_parse_alloyrag_summary_populated_after_processed(
    ...
    async def _run():
        ...
        try:
            ...
            assert not final_summary.startswith(ALLOYRAG_DOC_CONTENT_PREFIX), (
...
def test_raw_text_starting_with_marker_chunked_verbatim(tmp_path):
    ...
    characters.  ``strip_alloyrag_doc_prefix`` is a alloyrag-only contract
    ...
    body_with_marker = ALLOYRAG_DOC_CONTENT_PREFIX + (
    ...
    async def _run():
        ...
        assert spy["input"].startswith(ALLOYRAG_DOC_CONTENT_PREFIX), (
