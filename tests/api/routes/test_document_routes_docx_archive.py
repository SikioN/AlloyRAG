...
sys.argv = [sys.argv[0]]
_document_routes = importlib.import_module("alloyrag.api.routers.document_routes")
_alloyrag = importlib.import_module("alloyrag.alloyrag")
_pipeline = importlib.import_module("alloyrag.pipeline")
_base = importlib.import_module("alloyrag.base")
_constants = importlib.import_module("alloyrag.constants")
_utils = importlib.import_module("alloyrag.utils")
_parser_routing = importlib.import_module("alloyrag.parser.routing")
_parser_registry = importlib.import_module("alloyrag.parser.registry")
_parser_base = importlib.import_module("alloyrag.parser.base")
sys.argv = _original_argv

DocStatus = _base.DocStatus
DeletionResult = _base.DeletionResult
FULL_DOCS_FORMAT_ALLOYRAG = _constants.FULL_DOCS_FORMAT_ALLOYRAG
...
def _ensure_shared_storage_initialized():
    ...
    ``AlloyRAG`` instances don't run ``initialize_storages``, so we set
    up the shared store here and reset pipeline_status state per-test
    to avoid leakage.
    """
    import importlib

    shared_storage = importlib.import_module("alloyrag.kg.shared_storage")
...
class _ParseRag:
    _persist_parsed_full_docs = AlloyRAG._persist_parsed_full_docs
...
async def test_pipeline_index_file_leaves_alloyrag_document_docx_for_parser_archive(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("ALLOYRAG_PARSER", "docx:native")
...
async def test_pipeline_enqueue_alloyrag_document_docx_does_not_move_source(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("ALLOYRAG_PARSER", "docx:native")
...
async def test_pipeline_enqueue_docx_defers_to_legacy_parser(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALLOYRAG_PARSER", "docx:legacy")
...
async def test_pipeline_enqueue_parser_routed_pdf_defers_without_extraction(
    ...
    monkeypatch.setenv("ALLOYRAG_PARSER", "pdf:mineru,*:legacy")
...
async def test_pipeline_enqueue_alloyrag_parser_rule_provides_default_options(
    tmp_path, monkeypatch
):
    """ALLOYRAG_PARSER ``docx:native-iet`` becomes the default ``process_options``."""
...
async def test_pipeline_index_files_leaves_alloyrag_document_docx_batch(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("ALLOYRAG_PARSER", "docx:native")
...
async def test_clear_documents_sets_and_clears_destructive_busy(tmp_path):
    ...
    class _DropSpy:
        ...
        async def drop(self):
            shared_storage_inner = importlib.import_module("alloyrag.kg.shared_storage")
...
async def test_docx_archive_failure_is_best_effort(tmp_path, monkeypatch):
    ...
    import alloyrag.utils_pipeline as _utils_pipeline
...
async def test_parse_native_archives_docx_after_full_docs_sync(tmp_path, monkeypatch):
    ...
    # parse_native now returns ALLOYRAG-format parsed_data with merged_text
    # (not the {{LRdoc}} marker — that's only in the persisted full_docs row).
    assert result["content"]
    assert result["parse_format"] == "alloyrag"
    ...
    assert rag.full_docs.data["doc-test"]["parse_format"] == "alloyrag"
...
def test_parsed_artifact_dir_uses_unique_suffix_when_path_is_file(
    ...
    from alloyrag.utils_pipeline import parsed_artifact_dir_for
...
def test_alloyrag_document_reprocess_uses_full_docs_without_reparse():
    ...
    # All alloyrag rows route to the internal reuse handler (reuse the stored
...
def test_default_allowlist_equals_local_engine_suffixes(tmp_path, monkeypatch):
    ...
    from alloyrag.parser import registry
...
def test_unroutable_suffix_needs_rule_or_hint(tmp_path, monkeypatch):
    ...
    actually reaches that engine: a bare filename needs a ALLOYRAG_PARSER
    ...
    monkeypatch.setenv("ALLOYRAG_PARSER", "png:mineru")
...
def test_third_party_engine_suffixes_join_allowlist_and_scan(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setenv("ALLOYRAG_PARSER", "foo:fooengine")
...
class _ClearRag:
    """Mock AlloyRAG exposing the storages that clear_documents drops."""
