...
from alloyrag.api.passwords import BCRYPT_PASSWORD_PREFIX, hash_password
from alloyrag.tools.hash_password import main as hash_password_main
from alloyrag.utils import logger as alloyrag_logger
...
def auth_module(monkeypatch):
    config = import_real_api_module("alloyrag.api.config")
    ...
    module = import_real_api_module("alloyrag.api.auth")
    module = importlib.reload(module)
    yield module
    sys.modules.pop("alloyrag.api.auth", None)
...
def test_invalid_auth_accounts_raises(monkeypatch):
    ...
    with pytest.raises(ValueError, match="AUTH_ACCOUNTS must use"):
        import_real_api_module("alloyrag.api.auth")
...
def test_guest_tokens_fall_back_to_default_secret_when_token_secret_missing(
    ...
    monkeypatch.setattr(alloyrag_logger, "warning", capture_warning)
...
def _load_api_key_only_modules(monkeypatch):
    ...
    auth = import_real_api_module("alloyrag.api.auth")
    auth = importlib.reload(auth)
    utils_api = import_real_api_module("alloyrag.api.utils_api")
...
async def test_combined_auth_rejects_guest_token_in_api_key_mode(monkeypatch):
    ...
    try:
        ...
        sys.modules.pop("alloyrag.api.utils_api", None)
        sys.modules.pop("alloyrag.api.auth", None)
