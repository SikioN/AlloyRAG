...
def _isolate_env(monkeypatch):
    ...
    import alloyrag.api.config as config
...
def _build_client(monkeypatch, *, api_key=None):
    ...
    from alloyrag.api.config import parse_args, initialize_config
    ...
    import alloyrag.api.alloyrag_server as alloyrag_server
    ...
    monkeypatch.setattr(
        alloyrag_server, "create_document_routes", lambda *_a, **_k: APIRouter()
    )
    monkeypatch.setattr(
        alloyrag_server, "create_query_routes", lambda *_a, **_k: APIRouter()
    )
    monkeypatch.setattr(
        alloyrag_server, "create_graph_routes", lambda *_a, **_k: APIRouter()
    )
    monkeypatch.setattr(alloyrag_server, "OllamaAPI", _FakeOllamaAPI)
    monkeypatch.setattr(
        alloyrag_server, "get_namespace_data", AsyncMock(return_value={"busy": False})
    )
    ...
    app = alloyrag_server.create_app(args)
...
def _set_auth_mode(monkeypatch, *, auth_configured):
    ...
    """
    import alloyrag.api.utils_api as utils_api
...
# --------------------------------------------------------------------------- #
# Combined mode (AUTH_ACCOUNTS + ALLOYRAG_API_KEY): either a valid JWT or a
