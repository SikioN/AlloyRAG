...
# import time of alloyrag.api.config). Tests must be hermetic and not depend
...
_ENV_VARS_TO_ISOLATE = (
    ...
    "ALLOYRAG_API_PREFIX",
    "ALLOYRAG_KV_STORAGE",
    "ALLOYRAG_VECTOR_STORAGE",
    "ALLOYRAG_GRAPH_STORAGE",
    "ALLOYRAG_DOC_STATUS_STORAGE",
)
...
def _isolate_env(monkeypatch):
    ...
    The alloyrag.api.config module loads .env at import time, which can leave
...
def mock_args_api_prefix():
    ...
    from alloyrag.api.config import parse_args

    original_argv = sys.argv.copy()
    try:
        sys.argv = ["alloyrag-server", "--api-prefix", "/test-api"]
        args = parse_args()
        yield args
    finally:
...
class TestRootPathConfiguration:
    ...
    def test_root_path_set_when_prefix_provided(self, mock_args_api_prefix):
        ...
        with patch("alloyrag.api.alloyrag_server.AlloyRAG") as mock_rag:
            mock_rag.return_value = MagicMock()
            from alloyrag.api.alloyrag_server import create_app
...
class TestRoutesAtNaturalPaths:
    ...
    def test_document_routes_at_natural_path(self, mock_args_api_prefix):
        ...
        with patch("alloyrag.api.alloyrag_server.AlloyRAG") as mock_rag:
            ...
            # The route is mounted; the mocked AlloyRAG may cause 401/422/500,
...
class TestEnvironmentVariables:
    ...
    def test_env_api_prefix(self):
        """Test ALLOYRAG_API_PREFIX environment variable."""
        from alloyrag.api.config import get_env_value

        os.environ["ALLOYRAG_API_PREFIX"] = "unit-test-back/api"
        try:
            value = get_env_value("ALLOYRAG_API_PREFIX", "")
            assert value == "unit-test-back/api"
        finally:
            del os.environ["ALLOYRAG_API_PREFIX"]
...
class TestPathNormalization:
    ...
    def _build(self, *cli_args):
        # sys.argv must be the alloyrag-server form *before* alloyrag_server is
        # imported, because importing alloyrag.api.utils_api evaluates
        # `global_args.whitelist_paths` at module top level, which triggers
        # parse_args() against whatever sys.argv currently holds.
        original_argv = sys.argv.copy()
        try:
            sys.argv = ["alloyrag-server", *cli_args]
            from alloyrag.api.config import parse_args
            from alloyrag.api.alloyrag_server import create_app
...
class TestRuntimeConfigInjection:
    ...
    a `<script>window.__ALLOYRAG_CONFIG__ = {...}</script>` snippet on
    ...
    `alloyrag_server.__file__` so both `check_frontend_build()` and the
    ...
    PLACEHOLDER = "<!-- __ALLOYRAG_RUNTIME_CONFIG__ -->"
    ...
    def _build_app(self, tmp_path, monkeypatch, *cli_args):
        ...
        monkeypatch.setattr(sys, "argv", ["alloyrag-server", *cli_args])
        from alloyrag.api.config import parse_args
        from alloyrag.api import alloyrag_server
        from alloyrag.api.alloyrag_server import create_app

        # Redirect both check_frontend_build() and the StaticFiles mount to
        # our staged tmp directory.
        monkeypatch.setattr(
            alloyrag_server, "__file__", str(tmp_path / "alloyrag_server.py")
        )
    ...
    def test_injection_populates_window_config_with_prefix(self, tmp_path, monkeypatch):
        ...
        assert "window.__ALLOYRAG_CONFIG__" in body
    ...
    def test_missing_placeholder_serves_original_html(self, tmp_path, monkeypatch):
        ...
        assert "window.__ALLOYRAG_CONFIG__" not in response.text
...
class TestUvicornRootPathSemantics:
    ...
    def _build_app_with_prefix(self, prefix):
        original_argv = sys.argv.copy()
        try:
            sys.argv = ["alloyrag-server", "--api-prefix", prefix]
            from alloyrag.api.config import parse_args
            from alloyrag.api.alloyrag_server import create_app
    ...
    def test_launcher_does_not_pass_root_path_to_uvicorn(self, monkeypatch, tmp_path):
        ...
        Mocks uvicorn.run and exercises alloyrag_server.main() far enough
        to capture the config dict, then asserts root_path is absent.
        """
        monkeypatch.setenv("ALLOYRAG_API_PREFIX", "/site01")
        monkeypatch.setattr(sys, "argv", ["alloyrag-server"])
        ...
        monkeypatch.setattr(alloyrag_server, "uvicorn", MagicMock(run=fake_run))
        monkeypatch.setattr(alloyrag_server, "check_env_file", lambda: True)
        monkeypatch.setattr(
            alloyrag_server, "check_and_install_dependencies", lambda: None
        )
        monkeypatch.setattr(alloyrag_server, "configure_logging", lambda: None)
        monkeypatch.setattr(alloyrag_server, "update_uvicorn_mode_config", lambda: None)
        monkeypatch.setattr(alloyrag_server, "display_splash_screen", lambda *_: None)
        with patch("alloyrag.api.alloyrag_server.AlloyRAG") as mock_rag:
            mock_rag.return_value = MagicMock()
            # Re-parse args under the patched env so global_args picks up the prefix.
            from alloyrag.api.config import parse_args, global_args as _ga

            new_args = parse_args()
            for attr in vars(new_args):
                setattr(_ga, attr, getattr(new_args, attr))

            alloyrag_server.main()
    ...
    def test_gunicorn_uses_upstream_uvicorn_worker(self):
        ...
        """
        from alloyrag.api import gunicorn_config
