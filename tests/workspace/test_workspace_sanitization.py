...
to prevent Cypher injection via the ALLOYRAG-WORKSPACE HTTP header.
...
from the source files, as the full AlloyRAG package has many AI-related dependencies.
...
def get_actual_sanitization_logic():
    ...
    files = [
        os.path.join(base_path, "alloyrag/kg/memgraph_impl.py"),
        os.path.join(base_path, "alloyrag/kg/neo4j_impl.py"),
    ]
