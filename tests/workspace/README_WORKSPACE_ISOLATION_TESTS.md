...
## Overview
Comprehensive test coverage for AlloyRAG's workspace isolation feature, ensuring that different workspaces (projects) can coexist independently without data contamination or resource conflicts.
...
**Test:** 11
**Purpose:** Validate complete AlloyRAG workflows maintain isolation
...
ALLOYRAG_STRESS_TEST=true pytest tests/workspace/test_workspace_isolation.py -v

# Custom number of workers (e.g., 10)
ALLOYRAG_STRESS_TEST=true ALLOYRAG_TEST_WORKERS=10 pytest tests/workspace/test_workspace_isolation.py -v
```
...
ALLOYRAG_KEEP_ARTIFACTS=true pytest tests/workspace/test_workspace_isolation.py -v
```

#### Combined Example
```bash
# Stress test with 20 workers and keep artifacts
ALLOYRAG_STRESS_TEST=true \
ALLOYRAG_TEST_WORKERS=20 \
ALLOYRAG_KEEP_ARTIFACTS=true \
...
|----------|------|---------|-------------|
| `ALLOYRAG_STRESS_TEST` | bool | `false` | Enable stress testing mode |
| `ALLOYRAG_TEST_WORKERS` | int | `3` | Number of parallel workers in stress mode |
| `ALLOYRAG_KEEP_ARTIFACTS` | bool | `false` | Keep temporary test directories |
...
**Solution:** Run tests serially (`pytest -n 1`) or use `ALLOYRAG_KEEP_ARTIFACTS=true` to inspect state
...
2. **Run single test with artifacts:**
   ```bash
   ALLOYRAG_KEEP_ARTIFACTS=true pytest tests/workspace/test_workspace_isolation.py::test_json_kv_storage_workspace_isolation -v -s
   ```
...
- [Workspace Isolation Design Doc](../docs/AlloyRAG_concurrent_explain.md)
