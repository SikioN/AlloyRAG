...
from alloyrag.pipeline import _BatchRunContext
from alloyrag.parser.registry import parser_specs_snapshot
from alloyrag.utils import EmbeddingFunc, Tokenizer, get_content_summary
...
def _build_rag(tmp_path: Path) -> AlloyRAG:
...
async def _make_ctx(rag: AlloyRAG) -> _BatchRunContext:
...
async def _seed_doc_status(rag: AlloyRAG, doc_id: str, *, process_options: str = ""):
    now = datetime.now(timezone.utc).isoformat()
...
async def _drive_process_and_collect_chunks(
    rag: AlloyRAG, ctx: _BatchRunContext, doc_id: str
) -> tuple[dict, str]:
...
@pytest.mark.asyncio
async def test_process_strips_alloyrag_marker_on_reread(tmp_path):
    """For a alloyrag-format record, the re-read must strip the ``{{LRdoc}}``
    ...
    try:
        ...
        bare_body = "".join(f"AlloyRAG body line {i}. " for i in range(5))

        await rag.full_docs.upsert(
            {
                doc_id: {
                    # Stored WITH the marker, as parse persists alloyrag docs.
                    "content": make_alloyrag_doc_content(bare_body),
                    "file_path": f"{doc_id}.txt",
