...
These lock the ``AlloyRAG._insert_done`` / ``_discard_pending_index_ops`` /
...
backend flush is exercised — this is pure alloyrag.py logic.
"""
...
def _propagate_alloyrag_logs():
    """The ``alloyrag`` logger sets ``propagate=False``, so caplog's root
...
async def _make_rag(tmp_path) -> AlloyRAG:
    rag = AlloyRAG(
...
async def test_insert_done_multiple_failures_raises_first_logs_rest(
    ...
    try:
        ...
        with caplog.at_level("ERROR", logger="alloyrag"):
