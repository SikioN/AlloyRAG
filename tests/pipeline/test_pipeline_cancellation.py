...
from alloyrag.exceptions import MultimodalAnalysisError, PipelineCancelledException
...
def _build_rag(tmp_path: Path, *, vlm_func=None) -> AlloyRAG:
    role_configs = {}
    for spec in ROLES:
...
async def _shutdown_role_workers(rag: AlloyRAG) -> None:
    ...
    for func in rag.role_llm_funcs.values():
        try:
            ...
            logging.getLogger("alloyrag").warning(
                f"role worker shutdown raised during test teardown: {exc}"
            )


async def _make_ctx(rag: AlloyRAG) -> tuple[_BatchRunContext, dict, Any]:
...
async def test_parse_worker_drains_queue_when_cancelled_before_start(
    ...
    try:
        ...
        monkeypatch.setattr("alloyrag.pipeline.get_parser", get_parser_spy)
...
async def test_analyze_multimodal_inflight_cancellation_polls_flag(
    ...
    try:
        ...
        # NamespaceLock used during AlloyRAG initialization paths.
