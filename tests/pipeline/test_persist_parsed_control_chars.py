"""Injection point B: ``AlloyRAG._persist_parsed_full_docs`` strips C0
...
(``alloyrag.parser.debug.build_debug_rag``) so no real storage backends are
needed.
"""
...
async def test_persist_content_hash_matches_cleaned_body():
    ...
    expected = compute_text_content_hash(
        strip_alloyrag_doc_prefix("alphabeta", FULL_DOCS_FORMAT_RAW)
    )
