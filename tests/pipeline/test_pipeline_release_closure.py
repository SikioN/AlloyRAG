...
from alloyrag.base import DocStatus
...
from alloyrag.parser.registry import get_parser
...
def test_parse_engine_routing_by_filename_and_env(monkeypatch):
    ...
    monkeypatch.setenv("ALLOYRAG_PARSER", "pdf:mineru-iet,*:native")
...
def test_filename_hint_missing_required_endpoint_rejects(monkeypatch):
    from alloyrag.parser.routing import resolve_file_parser_directives
...
def test_validate_process_options_rejects_invalid_combos():
    from alloyrag.parser.routing import validate_process_options
...
@pytest.mark.offline
def test_alloyrag_parser_rule_supports_options_suffix(monkeypatch):
    monkeypatch.setenv("MINERU_LOCAL_ENDPOINT", "http://fake-mineru")
    monkeypatch.delenv("DOCLING_ENDPOINT", raising=False)
...
def test_resolve_file_parser_directives_priority(monkeypatch):
    ...
    monkeypatch.setenv("MINERU_LOCAL_ENDPOINT", "http://fake-mineru")
    monkeypatch.setenv("ALLOYRAG_PARSER", "docx:native-iet,*:legacy")
    ...
    # No filename hint → fall through to ALLOYRAG_PARSER defaults for both.
...
def test_doc_status_metadata_carry_over_helper():
    ...
    """
    from alloyrag.utils_pipeline import doc_status_transition_metadata
...
def test_carry_over_keys_grouped_by_stage():
    ...
    """
    from alloyrag.utils_pipeline import _DOC_STATUS_METADATA_CARRY_OVER_KEYS
...
def test_concurrent_enqueue_dedupes_same_content_different_filenames(tmp_path):
    ...
    async def _run():
        import alloyrag.pipeline as pipeline_module
...
def test_enqueue_rejects_removed_or_unknown_docs_format(tmp_path):
    """The 'alloyrag' ingestion entrypoint was removed: enqueue accepts only
    ...
    async def _run():
        ...
        try:
            with pytest.raises(ValueError, match="Unsupported docs_format"):
                await rag.apipeline_enqueue_documents(
                    "",
                    file_paths="first.alloyrag",
                    docs_format="alloyrag",
                )
            with pytest.raises(ValueError, match="Unsupported docs_format"):
                await rag.apipeline_enqueue_documents(
                    "some content",
            ...
            with pytest.raises(TypeError):
                await rag.apipeline_enqueue_documents(  # type: ignore[call-arg]
                    "",
                    file_paths="first.alloyrag",
                    alloyrag_document_paths="__parsed__/doc.blocks.jsonl",
                )
...
def test_pending_parse_duplicate_hash_fails_and_archives_source(tmp_path, monkeypatch):
    ...
    ``content_hash`` for ALLOYRAG-format docs is the MD5 of the normalized
    ...
    async def _run():
        ...
        try:
            ...
            import alloyrag.alloyrag as alloyrag_module
            import alloyrag.pipeline as pipeline_module
            ...
            monkeypatch.setattr(alloyrag_module, "datetime", _FrozenDateTime)
...
def test_three_phase_status_flow(tmp_path, monkeypatch):
    async def _run():
        ...
        monkeypatch.setattr("alloyrag.pipeline.merge_nodes_and_edges", _fake_merge)
...
def test_analyze_multimodal_invalid_json_hard_fails(tmp_path):
    ...
    async def _run():
        ...
        from alloyrag.exceptions import MultimodalAnalysisError
...
@pytest.mark.offline
def test_parse_mineru_to_alloyrag_document(tmp_path, monkeypatch):
    ...
    """
    from alloyrag.parser.external.mineru import compute_size_and_hash
    from alloyrag.parser.external.mineru.cache import current_mineru_options_signature
    from alloyrag.parser.external.mineru.client import MinerURawClient
    from alloyrag.parser.external.mineru.manifest import (
        Manifest,
        ManifestFile,
        write_manifest,
    )
    ...
    async def _run():
        ...
        assert parsed["parse_format"] == "alloyrag"
        ...
        assert full_doc["parse_format"] == "alloyrag"
...
def test_parse_mineru_uses_hint_source_and_canonical_upload_name(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        hinted_name = "AlloyRAG - Simple and Fast RAG.[mineru].pdf"
        canonical_name = "AlloyRAG - Simple and Fast RAG.pdf"
        ...
        expected_raw_dir = (
            input_dir
            / PARSED_DIR_NAME
            / ("AlloyRAG - Simple and Fast RAG.pdf.mineru_raw")
        )
...
def test_build_chunks_dict_preserves_existing_llm_cache_list():
    ...
    from alloyrag.utils_pipeline import build_chunks_dict_from_chunking_result
...
def test_strip_internal_multimodal_markup_cleans_table_id():
    ...
    from alloyrag.chunk_schema import (
...
def test_engine_params_survive_persist_to_full_docs(tmp_path, monkeypatch):
    ...
    """
    from alloyrag.parser.external.mineru import compute_size_and_hash
    from alloyrag.parser.external.mineru.cache import (
