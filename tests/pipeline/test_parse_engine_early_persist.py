...
from alloyrag.utils import EmbeddingFunc, Tokenizer, compute_mdhash_id
...
def test_resolve_engine_explicit_wins():
    ...
    assert (
        resolve_doc_status_parse_engine(FULL_DOCS_FORMAT_ALLOYRAG, PARSER_ENGINE_MINERU)
        == PARSER_ENGINE_MINERU
    )
    assert (
...
def test_resolve_engine_format_based_fallback():
    # No explicit engine: structured alloyrag → native, everything else
    # (raw passthrough, unknown) → legacy. Mirrors how a pre-engine corpus
    # was processed.
    assert (
        resolve_doc_status_parse_engine(FULL_DOCS_FORMAT_ALLOYRAG, None)
        == PARSER_ENGINE_NATIVE
    )
    assert (
...
def test_carry_over_preserves_parse_engine_and_format():
    ...
    assert carried["parse_format"] == FULL_DOCS_FORMAT_ALLOYRAG
...
def _new_rag(tmp_path: Path) -> AlloyRAG:
    return AlloyRAG(
...
def _attach_transition_spy(rag: AlloyRAG) -> list[tuple[str, dict]]:
...
def test_alloyrag_doc_records_native_engine_at_parsing(tmp_path):
    """A alloyrag-format doc on resume (already-parsed full_docs row pulled
    ...
    The row is seeded directly (full_docs ``parse_format=alloyrag`` +
    PENDING doc_status), mirroring how such rows exist in production now
    that the ``docs_format="alloyrag"`` enqueue entrypoint is removed:
    ...
    async def _run():
        ...
        try:
            ...
            _assert_early_and_stable(
                records,
                expected_format=FULL_DOCS_FORMAT_ALLOYRAG,
