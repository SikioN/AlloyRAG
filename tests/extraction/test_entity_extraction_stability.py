...
- ENTITY_TYPES env var raises SystemExit at AlloyRAG init
...
from alloyrag.utils import EmbeddingFunc, Tokenizer, TokenizerInterface
...
def _patch_prompt_dir(path: Path):
    return patch("alloyrag.prompt.get_entity_type_prompt_dir", return_value=path)
...
def test_default_entity_types_removed_from_constants():
    """DEFAULT_ENTITY_TYPES must no longer exist in alloyrag.constants."""
    import alloyrag.constants as constants
...
def test_entity_types_env_var_raises_system_exit(tmp_path):
    """AlloyRAG.__post_init__ must raise SystemExit when ENTITY_TYPES env var is set."""
...
async def test_text_mode_default_guidance_injected_into_prompt():
    ...
    from alloyrag.operate import extract_entities
    from alloyrag.prompt import PROMPTS
    ...
    with patch("alloyrag.operate.logger"):
...
async def test_rebuild_from_cached_fenced_json_uses_json_parser():
    ...
    from alloyrag import operate

    fenced_json = f"```json\n{_JSON_MODE_RESPONSE}\n```"

    with patch(
        "alloyrag.operate._process_extraction_result",
...
async def test_extract_entities_uses_cached_prompt_profile_without_reloading():
    ...
    with patch(
        "alloyrag.operate.resolve_entity_extraction_prompt_profile",
...
def test_prompt_dir_defaults_to_cwd_relative(tmp_path, monkeypatch):
    ...
    from alloyrag.prompt import get_entity_type_prompt_dir
...
def test_prompt_file_malformed_yaml_raises_valueerror(tmp_path):
    ...
    from alloyrag.prompt import load_entity_extraction_prompt_profile
...
def test_addon_guidance_overrides_file_profile(tmp_path):
    ...
    from alloyrag.prompt import resolve_entity_extraction_prompt_profile
...
def test_format_heading_context_full_path_includes_current_heading():
    ...
    from alloyrag.chunk_schema import format_heading_context
...
def test_format_parent_headings_per_level_cap_can_be_disabled():
    from alloyrag.chunk_schema import format_parent_headings
...
async def test_attach_content_headings_token_budgets_deep_path():
    ...
    from alloyrag.chunk_schema import HEADING_BREADCRUMB_SEP
    from alloyrag.constants import DEFAULT_MAX_SECTION_CONTEXT_TOKENS
    from alloyrag.operate import _attach_content_headings
...
def test_truncate_section_context_noop_within_budget():
    from alloyrag.operate import _truncate_section_context
...
async def test_extract_entities_bounds_pathological_heading_in_prompt():
    ...
    from alloyrag.chunk_schema import DEFAULT_HEADING_LEVEL_MAX_CHARS
    from alloyrag.operate import extract_entities
...
def test_clean_heading_text_converts_arrow_to_space():
    ...
    from alloyrag.chunk_schema import _clean_heading_text
