...
``AlloyRAG``'s synchronous wrappers (``insert``, ``query``,
``delete_by_entity`` …) all delegate to :func:`alloyrag.alloyrag._run_sync`,
...
storages were initialized on (``AlloyRAG._owning_loop``).  Two misuse modes
break this:
...
* Driven from a different — but still alive — loop, e.g.
  ...
  the shared ``asyncio.Lock`` objects in ``alloyrag.kg.shared_storage`` to the
...
from alloyrag.alloyrag import _run_sync
