...
from alloyrag.kg.qdrant_impl import (  # noqa: E402
...
def patch_namespace_lock():
    ...
    with patch("alloyrag.kg.qdrant_impl.get_namespace_lock", side_effect=factory):
        yield cache
...
def _make_storage(
    ...
    *,
    ...
    storage.final_namespace = f"alloyrag_vdb_{namespace}_mock"
    ...
    from alloyrag.kg.qdrant_impl import get_namespace_lock
...
async def test_drop_clears_workspace_points_from_workspace_tagged_legacy():
    ...
    embed = CountingEmbeddingFunc()
    s = _make_storage(embed)
    legacy_collection = f"alloyrag_vdb_{s.namespace}"
