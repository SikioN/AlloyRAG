...
from alloyrag.kg.faiss_impl import FaissVectorDBStorage  # noqa: E402
from alloyrag.kg.shared_storage import (  # noqa: E402
...
async def test_reload_warns_on_index_meta_skew(tmp_path, caplog):
    ...
    # The alloyrag logger sets propagate=False (alloyrag/utils.py), so caplog —
    # which attaches to root by default — never sees its records. Flip propagate
    # for the duration of the reload, then restore.
    caplog.clear()
    old_propagate = alloyrag_logger.propagate
    alloyrag_logger.propagate = True
    try:
        with caplog.at_level(logging.WARNING, logger="alloyrag"):
            reader = _make_storage(tmp_path, embed)
            await reader.initialize()
    finally:
        alloyrag_logger.propagate = old_propagate
