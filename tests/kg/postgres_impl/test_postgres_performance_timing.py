...
import alloyrag.utils as utils_module
from alloyrag.kg.postgres_impl import PGGraphStorage, PostgreSQLDB
...
async def test_execute_timing_logs_success():
    ...
    with patch("alloyrag.kg.postgres_impl.performance_timing_log") as timing_log:
...
def test_performance_timing_logs_reads_new_env_only(monkeypatch):
    with monkeypatch.context() as m:
        m.setenv("ALLOYRAG_DOC_QUERY_TIMING_LOGS", "false")
        m.setenv("ALLOYRAG_PERFORMANCE_TIMING_LOGS", "true")
...
def test_performance_timing_logs_ignores_old_env(monkeypatch):
    with monkeypatch.context() as m:
        m.setenv("ALLOYRAG_DOC_QUERY_TIMING_LOGS", "true")
        m.setenv("ALLOYRAG_PERFORMANCE_TIMING_LOGS", "false")
        reloaded = importlib.reload(utils_module)
