"""
Pytest configuration for AlloyRAG tests.
...
def _hermetic_mineru_env(monkeypatch):
    ...
    ``alloyrag/api/{auth,config}.py`` call ``load_dotenv(override=False)``
    ...
    ``ALLOYRAG_PARSER`` is cleared for the same reason: a routing rule
...
def pytest_configure(config):
    """Register custom markers for AlloyRAG tests."""
    ...
    config.addinivalue_line(
        "markers", "requires_api: marks tests requiring AlloyRAG API server"
    )


def pytest_addoption(parser):
    """Add custom command-line options for AlloyRAG tests."""
...
def keep_test_artifacts(request):
    ...
    return os.getenv("ALLOYRAG_KEEP_ARTIFACTS", "false").lower() == "true"
...
def stress_test_mode(request):
    ...
    # Fall back to environment variable
    return os.getenv("ALLOYRAG_STRESS_TEST", "false").lower() == "true"
...
def parallel_workers(request):
    ...
    # Fall back to environment variable
    return int(os.getenv("ALLOYRAG_TEST_WORKERS", "3"))
...
def run_integration_tests(request):
    ...
    # Fall back to environment variable
    return os.getenv("ALLOYRAG_RUN_INTEGRATION", "false").lower() == "true"
