...
prompt_with_default() {{
  case "$1" in
    ...
    "PostgreSQL user") printf 'alloyrag' ;;
    "PostgreSQL database") printf 'alloyrag' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    ...
    assert values["POSTGRES_DATABASE"] == "alloyrag"
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL user[rag]|secret:PostgreSQL password: [rag]|PostgreSQL database[alloyrag]"
    )
...
""")
    ...
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL port[5432]|PostgreSQL user[rag]|PostgreSQL password: [rag]|PostgreSQL database[alloyrag]"
    )
...
prompt_with_default() {{
  case "$1" in
    ...
    "MongoDB database") printf 'AlloyRAG' ;;
    "Milvus database name") printf 'alloyrag' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    assert values["MILVUS_DB_NAME"] == "alloyrag"
...
ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]="MongoVectorDBStorage"
ENV_VALUES[MONGO_URI]="mongodb://atlas-local.example.com:27017/AlloyRAG?replicaSet=rs0&directConnection=true"
...
""")
    assert (
        values["MONGO_URI"]
        == "mongodb://atlas-local.example.com:27017/AlloyRAG?replicaSet=rs0&directConnection=true"
    )
    assert values["COMPOSE_MONGO_URI"] == ""
...
""")
    ...
    assert values["ALLOYRAG_API_KEY_SET"] == ""
    ...
    assert not any(line.startswith("ALLOYRAG_API_KEY=") for line in generated_lines)
