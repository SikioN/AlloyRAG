...
def test_generate_docker_compose_removes_alloyrag_env_file_to_preserve_dollar_values(
    tmp_path: Path,
) -> None:
...
def test_generate_docker_compose_removes_alloyrag_container_name_from_existing_output(
    tmp_path: Path,
) -> None:
    """Compose regeneration should strip fixed alloyrag container names from prior output."""
    write_text_lines(
...
def test_generate_docker_compose_preserves_list_style_alloyrag_environment(
    tmp_path: Path,
) -> None:
...
def test_generate_docker_compose_injects_healthchecks_and_alloyrag_depends_on(
    tmp_path: Path,
) -> None:
    """Generated compose should gate AlloyRAG on all managed dependencies becoming healthy."""
    write_text_lines(
        tmp_path / "docker-compose.yml",
...
""")
    ...
    alloyrag_start = generated_compose.index("  alloyrag:\n")
    embed_start = generated_compose.index("\n  vllm-embed:\n")
    alloyrag_block = generated_compose[alloyrag_start:embed_start]
    assert "    depends_on:" in generated_compose
    assert "    depends_on:" in alloyrag_block
    for service_name in (
        "postgres",
        "neo4j",
        "mongodb",
...
def test_generate_docker_compose_repairs_misplaced_alloyrag_depends_on_from_existing_output(
    tmp_path: Path,
) -> None:
    """Regeneration should move stale alloyrag depends_on content back onto the alloyrag service."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
""")
    ...
    alloyrag_block = generated_compose[alloyrag_start:rerank_start]
...
def test_generate_docker_compose_normalizes_alloyrag_restart_policy_from_existing_output(
    tmp_path: Path,
) -> None:
    """Regeneration should replace legacy alloyrag restart with deploy.restart_policy."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
""")
    ...
    alloyrag_start = generated_compose.index("  alloyrag:\n")
    alloyrag_block = generated_compose[alloyrag_start:]
    assert "    restart: unless-stopped" not in alloyrag_block
    assert "    deploy:\n" in alloyrag_block
...
def test_generate_docker_compose_normalizes_alloyrag_restart_policy_without_blank_line_before_deploy(
    tmp_path: Path,
) -> None:
...
def test_generate_docker_compose_cleans_marker_and_blank_lines_when_only_alloyrag_remains(
    tmp_path: Path,
) -> None:
...
ENV_VALUES[ALLOYRAG_API_KEY]='plain$token'
...
def test_generate_docker_compose_injects_env_overrides_into_alloyrag_not_after_managed_services(
    tmp_path: Path,
) -> None:
    """Env overrides must appear inside the alloyrag environment block, not after managed services.
    ...
    the alloyrag service once postgres/neo4j were merged in.
    """
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text(
        "\n".join(
