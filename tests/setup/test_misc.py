...
def test_find_generated_compose_file_prefers_final_compose_file(tmp_path: Path) -> None:
    ...
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        ["services:", "  alloyrag:", "    image: final/alloyrag"],
    )
    write_text_lines(
        tmp_path / "docker-compose.development.yml",
        ["services:", "  alloyrag:", "    image: dev/alloyrag"],
    )
    write_text_lines(
        tmp_path / "docker-compose.production.yml",
        ["services:", "  alloyrag:", "    image: prod/alloyrag"],
    )
    output = run_bash(f"""
set -euo pipefail
source "{REPO_ROOT}/scripts/setup/setup.sh"
...
def test_finalize_base_setup_uses_compose_native_storage_endpoints_on_rerun(
    ...
    write_text_lines(
        ...
        [
            ...
            "ALLOYRAG_SETUP_MILVUS_DEPLOYMENT=docker",
...
ORIGINAL_ENV_VALUES[POSTGRES_DATABASE]="alloyrag"
...
ENV_VALUES[ALLOYRAG_SETUP_MONGODB_DEPLOYMENT]="{deployment_marker}"
...
ENV_VALUES[ALLOYRAG_SETUP_MONGODB_DEPLOYMENT]="docker"
EXISTING_MANAGED_ROOT_SERVICE_SET[mongodb]=1
DOCKER_SERVICE_SET[mongodb]=1
...
def test_finalize_server_setup_skips_embedded_milvus_sub_services(
    ...
    write_text_lines(tmp_path / ".env", ["ALLOYRAG_SETUP_MILVUS_DEPLOYMENT=docker"])
    run_bash(f"""
set -euo pipefail
source "{REPO_ROOT}/scripts/setup/setup.sh"
REPO_ROOT="{tmp_path}"
...
def test_finalize_server_setup_uses_compose_native_neo4j_endpoint_on_rerun(
    ...
    write_text_lines(
        tmp_path / ".env",
        ["ALLOYRAG_SETUP_NEO4J_DEPLOYMENT=docker", "NEO4J_URI=neo4j://localhost:7687"],
    )
    write_text_lines(
        tmp_path / "env.example",
        (REPO_ROOT / "env.example").read_text(encoding="utf-8").splitlines(),
    ...
    [
        ...
        (
            ...
            [
                ...
                "POSTGRES_USER=alloyrag",
                "POSTGRES_PASSWORD=secret",
                "POSTGRES_DATABASE=alloyrag",
            ],
...
def test_warn_if_exposed_silent_when_auth_configured() -> None:
    ...
    result = _run_warn_if_exposed(
        'ENV_VALUES[HOST]="0.0.0.0"\nENV_VALUES[ALLOYRAG_API_KEY]="some-key"'
    )
    assert result.stdout.strip() == ""
...
def test_security_check_reports_api_key_only_with_default_whitelist(
    ...
    write_text_lines(tmp_path / ".env", ["ALLOYRAG_API_KEY=my-secret-key"])
    result = subprocess.run(
        [
            "bash",
            "--norc",
...
def test_security_check_reports_api_key_only_with_explicit_api_wildcard_whitelist(
    ...
    write_text_lines(
        tmp_path / ".env",
        ["ALLOYRAG_API_KEY=my-secret-key", "WHITELIST_PATHS=/health,/api/*"],
    )
    result = subprocess.run(
        [
            "bash",
...
def test_security_check_reports_api_key_only_with_catch_all_whitelist(
    ...
    write_text_lines(
        tmp_path / ".env",
        ["ALLOYRAG_API_KEY=my-secret-key", "WHITELIST_PATHS=/*"],
    )
    result = subprocess.run(
        [
            "bash",
...
def test_security_check_passes_for_api_key_only_with_apiary_whitelist(
    ...
    write_text_lines(
        tmp_path / ".env",
        ["ALLOYRAG_API_KEY=my-secret-key", "WHITELIST_PATHS=/health,/apiary/*"],
    )
    result = subprocess.run(
        [
            "bash",
...
def test_security_check_passes_for_api_key_only_with_safe_whitelist(
    ...
    write_text_lines(
        tmp_path / ".env", ["ALLOYRAG_API_KEY=my-secret-key", "WHITELIST_PATHS=/health"]
    )
    result = subprocess.run(
        [
            "bash",
...
def test_security_check_ignores_default_opensearch_password_when_opensearch_unused(
    ...
    write_text_lines(
        ...
        [
            ...
            "ALLOYRAG_GRAPH_STORAGE=NetworkXStorage",
            "ALLOYRAG_DOC_STATUS_STORAGE=JsonDocStatusStorage",
            "OPENSEARCH_PASSWORD=AlloyRAG2026_!@",
        ],
    )
    result = subprocess.run(
        [
...
def test_backup_only_backs_up_env_and_generated_compose(tmp_path: Path) -> None:
    ...
    compose_content = (
        "\n".join(["services:", "  alloyrag:", "    image: example/alloyrag:test"])
        + "\n"
    )
    write_text_lines(tmp_path / ".env", ["HOST=0.0.0.0"])
