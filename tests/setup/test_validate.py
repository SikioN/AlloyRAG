...
def test_validate_env_file_rejects_container_ssl_paths_for_default_host_target(
    ...
    """Omitting ALLOYRAG_RUNTIME_TARGET defaults to host; container paths must still be rejected."""
    (tmp_path / "data" / "certs").mkdir(parents=True)
    (tmp_path / "data" / "certs" / "cert.pem").write_text("cert", encoding="utf-8")
    (tmp_path / "data" / "certs" / "key.pem").write_text("key", encoding="utf-8")
    env_file = tmp_path / ".env"
...
def test_validate_env_file_handles_supported_and_unsupported_uri_schemes(
    ...
    cases = {
        "invalid-neo4j-scheme": (
            [
                "ALLOYRAG_GRAPH_STORAGE=Neo4JStorage",
        ...
        "invalid-redis-scheme": (
            ["ALLOYRAG_KV_STORAGE=RedisKVStorage", "REDIS_URI=tcp://localhost:6379"],
            "no",
            "Invalid REDIS_URI",
        ),
        "valid-rediss-scheme": (
            ["ALLOYRAG_KV_STORAGE=RedisKVStorage", "REDIS_URI=rediss://localhost:6380"],
            "yes",
            "",
        ),
    }
...
def test_validate_env_file_rejects_invalid_runtime_target(tmp_path: Path) -> None:
    """validate_env_file should reject unsupported ALLOYRAG_RUNTIME_TARGET values."""
    write_text_lines(
        tmp_path / ".env",
        [
            "ALLOYRAG_RUNTIME_TARGET=laptop",
            "ALLOYRAG_KV_STORAGE=JsonKVStorage",
            "ALLOYRAG_VECTOR_STORAGE=NanoVectorDBStorage",
            "ALLOYRAG_GRAPH_STORAGE=NetworkXStorage",
            "ALLOYRAG_DOC_STATUS_STORAGE=JsonDocStatusStorage",
...
""",
    ...
    values = parse_lines(result.stdout)
    assert values["VALID"] == "no"
    assert "Invalid ALLOYRAG_RUNTIME_TARGET" in result.stderr
...
if validate_required_variables   "${{ENV_VALUES[ALLOYRAG_KV_STORAGE]}}"   "${{ENV_VALUES[ALLOYRAG_VECTOR_STORAGE]}}"   "${{ENV_VALUES[ALLOYRAG_GRAPH_STORAGE]}}"   "${{ENV_VALUES[ALLOYRAG_DOC_STATUS_STORAGE]}}"; then
  printf 'VALID=yes\\n'
else
  printf 'VALID=no\\n'
fi
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "ALLOYRAG_GRAPH_STORAGE=MongoGraphStorage",
                "ALLOYRAG_DOC_STATUS_STORAGE=MongoDocStatusStorage",
                "MONGO_URI=mongodb://atlas-local.example.com:27017/AlloyRAG?replicaSet=rs0&directConnection=true",
                "MONGO_DATABASE=AlloyRAG",
            ]
        )
        + "\n",
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "ALLOYRAG_GRAPH_STORAGE=OpenSearchGraphStorage",
                "ALLOYRAG_DOC_STATUS_STORAGE=OpenSearchDocStatusStorage",
                "ALLOYRAG_SETUP_OPENSEARCH_DEPLOYMENT=docker",
                "OPENSEARCH_HOSTS=localhost:9200",
                "OPENSEARCH_USER=admin",
            ]
        )
