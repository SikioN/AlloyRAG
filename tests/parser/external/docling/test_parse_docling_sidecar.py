...
- ``ALLOYRAG_FORCE_REPARSE_DOCLING=true`` forces a re-download even when
...
from alloyrag.constants import FULL_DOCS_FORMAT_ALLOYRAG
from alloyrag.parser.external import (
    Manifest,
    ManifestFile,
    compute_size_and_hash,
    write_manifest,
)
from alloyrag.parser.external.docling.cache import (
    compute_options_signature,
    snapshot_tunable_env,
)
from alloyrag.parser.external.docling.client import FIXED_CONSTANTS
...
def _install_fake_download(monkeypatch: pytest.MonkeyPatch) -> dict[str, int]:
    ...
    import alloyrag.parser.external.docling.client as client_mod
...
def _stub_pipeline(monkeypatch: pytest.MonkeyPatch, rag: AlloyRAG, src: Path) -> None:
...
def _seed_doc_status(rag: AlloyRAG, doc_id: str) -> Any:
...
def test_parse_docling_emits_compliant_sidecar(
    ...
    async def _run() -> None:
        ...
        try:
            ...
            assert parsed["parse_format"] == FULL_DOCS_FORMAT_ALLOYRAG
...
def test_parse_docling_cache_hit_skips_download(
    ...
    async def _run() -> None:
        ...
        try:
            ...
            monkeypatch.setenv("ALLOYRAG_FORCE_REPARSE_DOCLING", "true")
            await _parse_via_registry(
                rag,
                "docling",
                doc_id=doc_id,
