"""End-to-end test: native docx → AlloyRAG Document → stable cache key.
...
``full_docs`` is in ALLOYRAG format. ``_load_alloyrag_document_content``
...
from alloyrag.utils import Tokenizer, TokenizerInterface, compute_args_hash
...
@pytest.mark.offline
def test_native_alloyrag_path_produces_stable_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        # AlloyRAG-specific writing — that is what we want under test.
        stable_blocks = [
            _block(
        ...
        assert record["parse_format"] == "alloyrag"
...
@pytest.mark.offline
def test_native_alloyrag_path_writes_blocks_jsonl_and_skips_meta_on_load(
    tmp_path, monkeypatch
):
    """Sanity check: ``_load_alloyrag_document_content`` must skip the
...
@pytest.mark.offline
def test_native_alloyrag_path_leaves_unknown_table_caption_empty(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        from alloyrag.multimodal_context import enrich_sidecars_with_surrounding
...
@pytest.mark.offline
def test_native_alloyrag_path_writes_image_assets_to_blocks_assets_dir(
    tmp_path, monkeypatch
):
