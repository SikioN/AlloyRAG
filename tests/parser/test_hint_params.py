...
assertions are independent of any ambient ``ALLOYRAG_PARSER``.
"""
...
sys.argv = [sys.argv[0]]
importlib.import_module("alloyrag.api.routers.document_routes")
...
# --------------------------------------------------------------------------- #
# routing: ALLOYRAG_PARSER rule params + overlay merge
# --------------------------------------------------------------------------- #
...
def test_upload_path_applies_hint_chunk_params(tmp_path):
    ...
    from alloyrag.api.routers.document_routes import pipeline_enqueue_file
