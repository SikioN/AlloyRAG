"""Tests for the unified parser debug CLI (``alloyrag/parser/cli.py``).
...
from alloyrag.parser.cli import main
...
def _clean_env(monkeypatch: pytest.MonkeyPatch) -> None:
    for name in (
        ...
        "ALLOYRAG_FORCE_REPARSE_DOCLING",
    ):
...
def test_cli_direct_script_native_does_not_shadow_python_docx(
    ...
    cli_path = repo_root / "alloyrag" / "parser" / "cli.py"
    proc = subprocess.run(
        [
            sys.executable,
            str(cli_path),
