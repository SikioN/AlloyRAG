...
from alloyrag.parser.markdown import parser as md_parser
...
def _enable_download(monkeypatch):
    ...
    monkeypatch.delenv("ALLOYRAG_FORCE_REPARSE_NATIVE", raising=False)
...
def test_force_reparse_discards_cache(tmp_path, monkeypatch):
    ...
    _extract(p, src, parsed)
    assert counter["n"] == 1
    monkeypatch.setenv("ALLOYRAG_FORCE_REPARSE_NATIVE", "true")
    _wipe_parsed(parsed)
    _extract(p, src, parsed)
    assert counter["n"] == 2
