...
frameworks that build on top of AlloyRAG (like RAGAnything).
...
async def main():
    ...
    # os.environ["MILVUS_DB_NAME"] = "alloyrag"

    # Initialize AlloyRAG with Milvus index configuration via vector_db_storage_cls_kwargs
    ...
    print(
        "✅ AlloyRAG initialized with Milvus index configuration via vector_db_storage_cls_kwargs"
    )
    print(
    ...
    sample_text = """
    AlloyRAG is a Retrieval-Augmented Generation framework that uses graph-based
    ...
    result = await rag.aquery("What is AlloyRAG?", param=QueryParam(mode="hybrid"))
