"""AlloyRAG + AG2 Multi-Agent Demo.

Demonstrates how AG2 agents can use AlloyRAG's knowledge graph retrieval
as a tool. Multiple specialized agents collaborate to answer complex
questions over indexed documents.

Architecture:
    User -> AG2 GroupChat (Researcher + Analyst + Writer) -> AlloyRAG queries
    - Researcher: uses AlloyRAG hybrid search to gather facts
    - Analyst: uses AlloyRAG naive (vector) search for complementary results
    - Writer: synthesizes findings into a final answer

Requires:
    pip install alloyrag-hku "ag2[openai]>=0.11.4,<1.0"
    export OPENAI_API_KEY="..."

Usage:
    python examples/alloyrag_ag2_multiagent_demo.py
"""
...
AlloyRAG implements this approach with dual-level retrieval: local search focuses
...
async def setup_alloyrag() -> AlloyRAG:
    """Initialize AlloyRAG and index sample documents."""
    ...
    print("AlloyRAG initialized and documents indexed.\n")
    return rag


# --- Async Bridge ---
# AG2 runs tools in a background thread without an event loop.
# We maintain a dedicated event loop in a separate thread for AlloyRAG async calls.
...
# Global reference to AlloyRAG instance (set in main)
_rag_instance: AlloyRAG = None


def create_agents():
    """Create AG2 agents with AlloyRAG tools."""
    ...
    researcher = AssistantAgent(
        name="Researcher",
        system_message=(
            "You are a research specialist. Use the alloyrag_query tool to search "
    ...
    analyst = AssistantAgent(
        ...
        system_message=(
            ...
            "the alloyrag_query tool with mode='naive' to run a direct vector search. "
    ...
    # --- Register AlloyRAG as a tool ---

    @user_proxy.register_for_execution()
    @researcher.register_for_llm(
        description=(
            "Query the AlloyRAG knowledge base. "
    ...
    def alloyrag_query(query: str, mode: str = "hybrid") -> str:
        """Query AlloyRAG synchronously (wraps async call)."""
...
def run_multiagent_query(user_proxy, researcher, analyst, writer, question: str):
    """Run a multi-agent GroupChat to answer a question using AlloyRAG."""
...
def main():
    ...
    # Start a background event loop for AlloyRAG async calls.
    ...
    try:
        # Step 1: Set up AlloyRAG (async, runs on the background loop)
        _rag_instance = _run_async(setup_alloyrag())

        # Step 2: Create AG2 agents with AlloyRAG tools
