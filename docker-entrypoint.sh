...
# Run the server as the non-root "alloyrag" user (CIS Docker 4.1) while staying
...
if [ "${1#-}" != "$1" ]; then
    set -- python -m alloyrag.api.alloyrag_server "$@"
fi
...
if [ "$(id -u)" = "0" ]; then
    ...
    # handing it to alloyrag avoids that PermissionError.
    #
    ...
    for _d in /app/data "$WORKING_DIR" "$INPUT_DIR" "$PROMPT_DIR" \
        ...
        chown -R alloyrag:alloyrag "$_d" 2>/dev/null || true
    done
